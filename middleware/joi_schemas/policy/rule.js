    ...
    return new Promise(async(resolve, reject) => {
        ...
        if (req.method === 'POST' || (req.method === 'PUT' && req.url === '/policy/rule')) {
            ...
            if (req.url === '/policy/rule/type/get')
                ...
                schema = schema.append({ pasteOnRuleId: sharedSch.id, pasteOffset: Joi.number().integer(), pasteOnRuleOrder: Joi.number().integer(), action: sharedSch.rule_clipboard_action, rulesIds: Joi.array().items(sharedSch.id), idgroup: sharedSch.id.allow(null).optional() });
