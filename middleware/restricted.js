...
restrictedCheck.interface = (req, res, next) => {
    ...
    interfaceModel.searchInterfaceInrulesPro(req.body.firewall, type, req.body.fwcloud, '')
        .then(data => {
            //CHECK RESULTS
            if (data.result) {
...
restrictedCheck.ipobj = (req, res, next) => {
    ipobjModel.searchIpobjInRules(req.body.firewall, req.body.type, req.body.fwcloud)
        .then(data => {
            if (data.result) {
                const restricted = { "result": false, "msg": "Restricted", "restrictions": data.search };
                api_resp.getJson(restricted, api_resp.ACR_RESTRICTED, 'RESTRICTED', null, null, jsonResp => res.status(200).json(jsonResp));
...
restrictedCheck.ipobj_group = (req, res, next) => {
    ipobj_gModel.searchGroupInRules(req.body.firewall, req.body.fwcloud)
        .then(data => {
            if (data.result) {
                const restricted = { "result": false, "msg": "Restricted", "restrictions": data.search };
                api_resp.getJson(restricted, api_resp.ACR_RESTRICTED, 'RESTRICTED', null, null, jsonResp => res.status(200).json(jsonResp));
