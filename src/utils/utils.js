...
utilsModel.disableFirewallCompileStatus = (req, res, next) => {
  var firewall = 0;
  if (req.body.firewall) firewall = req.body.firewall;
  else if (req.body.rulesData) firewall = req.body.rulesData.firewall;
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (data.install_pass !== null) {
        var decipherPass = crypto.createDecipher(
          config.get('crypt').algorithm,
          config.get('crypt').secret,
        );
...
utilsModel.deleteFile = (dir, file) => {
  return new Promise((resolve, reject) => {
    ...
    fs.lstat(filePath, (err, stats) => {
      ...
      if (stats.isDirectory()) resolve(utilsModel.deleteFolder(filePath));
      else {
  ...
  return new Promise((resolve, reject) => {
    fs.access(dir, (err) => {
      ...
      fs.readdir(dir, (err, files) => {
        ...
        Promise.all(
          files.map((file) => {
            return utilsModel.deleteFile(dir, file);
          }),
        )
          .then(() => {
            fs.rmdir(dir, (err) => {
