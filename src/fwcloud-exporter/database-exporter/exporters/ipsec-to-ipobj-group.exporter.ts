...
export class IPSecToIPObjGroupExporter extends TableExporter {
  protected getEntity(): typeof Model {
    return null;
  }

  public getTableName(): string {
    return 'ipsec__ipobj_g';
  }
  ...
  protected async getRows(connection: DataSource, fwCloudId: number): Promise<any> {
    ...
    const data = await qr.query(
      `SELECT * FROM ${this.getTableName()} 
        WHERE ipsec IN ${this.getIPSecIds(connection, fwCloudId)[0]}
        OR ipobj_g IN ${this.getIpObjGruopIds(connection, fwCloudId)[0]}`,
      this.getIPSecIds(connection, fwCloudId)[1].concat(
  ...
  protected getIPSecIds(connection: DataSource, fwCloudId: number): [string, Array<any>] {
    const subquery = connection
      .createQueryBuilder()
      .subQuery()
      .from(IPSec, 'ipsec')
      .select('ipsec.id');
    return new IPSecExporter()
      .getFilterBuilder(subquery, 'ipsec', fwCloudId)
