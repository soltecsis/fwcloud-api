...
import { IPSecPrefixExporter } from './ipsec-prefix.exporter';
import { IPObjGroupExporter } from './ipobj-group.exporter';
import { IPObjGroup } from '../../../models/ipobj/IPObjGroup';

export class IPSecPrefixToIPObjGroupExporter extends TableExporter {
  protected getEntity(): typeof Model {
    return null;
  }

  public getTableName(): string {
    return 'ipsec_prefix__ipobj_g';
  }
  ...
  protected async getRows(connection: DataSource, fwCloudId: number): Promise<any> {
    ...
    const data = await qr.query(
      `SELECT * FROM ${this.getTableName()} 
        WHERE prefix IN ${this.getIPSecPrefixIds(connection, fwCloudId)[0]}
        OR ipobj_g IN ${this.getIpObjGruopIds(connection, fwCloudId)[0]}`,
      this.getIPSecPrefixIds(connection, fwCloudId)[1].concat(
  ...
  protected getIPSecPrefixIds(connection: DataSource, fwCloudId: number): [string, Array<any>] {
    const subquery = connection
      .createQueryBuilder()
      .subQuery()
      .from(IPSecPrefix, 'ipsec_prefix')
      .select('ipsec_prefix.id');
    return new IPSecPrefixExporter()
      .getFilterBuilder(subquery, 'ipsec_prefix', fwCloudId)
