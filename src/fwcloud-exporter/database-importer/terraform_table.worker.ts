...
async function terraformTable(
  tableName: string,
  mapper: ImportMapping,
  data: object[],
): Promise<object[]> {
  return await new Terraformer(
    mapper,
    this ? this.eventEmitter : new EventEmitter(),
...
terraformTable(
  sharedData.tableName,
  mapper,
  result.getTableResults(sharedData.tableName),
)
  .then((data) => {
