...
export class RoutingRuleController extends Controller {
  ...
  public async make(request: Request): Promise<void> {
    ...
    if (request.params.routingRule) {
      this._routingRule = await getRepository(RoutingRule).findOneOrFail(
        parseInt(request.params.routingRule),
      );
    }
  ...
  async show(request: Request): Promise<ResponseBuilder> {
    (
      await RoutingRulePolicy.show(this._routingRule, request.session.user)
    ).authorize();
  ...
  async create(request: Request): Promise<ResponseBuilder> {
    (
      await RoutingRulePolicy.create(this._firewall, request.session.user)
    ).authorize();

    const rule: RoutingRule = await this.routingRuleService.create(
      request.inputs.all<ICreateRoutingRule>(),
    );
  ...
  async copy(request: Request): Promise<ResponseBuilder> {
    ...
    for (const id of ids) {
      const rule: RoutingRule =
        await this.routingRuleService.findOneInPathOrFail({
          fwCloudId: this._fwCloud.id,
          firewallId: this._firewall.id,
          id: parseInt(id),
        });
  ...
  async update(request: Request): Promise<ResponseBuilder> {
    (
      await RoutingRulePolicy.update(this._routingRule, request.session.user)
    ).authorize();
  ...
  async bulkUpdate(request: Request): Promise<ResponseBuilder> {
    ...
    const result: RoutingRule[] = await this.routingRuleService.bulkUpdate(
      rules.map((item) => item.id),
      request.inputs.all(),
    );
  ...
  async moveFrom(request: Request): Promise<ResponseBuilder> {
    ...
    const fromRule: RoutingRule = await getRepository(
      RoutingRule,
    ).findOneOrFail({
      join: {
        alias: "rule",
    ...
    const result: RoutingRule[] = await this.routingRuleService.moveFrom(
      fromRule.id,
      toRule.id,
      request.inputs.all(),
    );
  ...
  async remove(request: Request): Promise<ResponseBuilder> {
    (
      await RoutingRulePolicy.delete(this._routingRule, request.session.user)
    ).authorize();
  ...
  async bulkRemove(request: Request): Promise<ResponseBuilder> {
    ...
    const returned: RoutingRule[] = await this.routingRuleService.bulkRemove(
      rules.map((item) => item.id),
    );
