...
export class RouteController extends Controller {
  ...
  async move(request: Request): Promise<ResponseBuilder> {
    ...
    const routes: Route[] = await db
      ...
      .where('route.id IN (:...ids)', { ids: request.inputs.get<number[]>('routes') })
  ...
  async update(request: Request): Promise<ResponseBuilder> {
    ...
    const result: Route = await this._routeService.update(
      this._route.id,
      request.inputs.all<RouteControllerUpdateDto>(),
    );
  ...
  async bulkUpdate(request: Request): Promise<ResponseBuilder> {
    ...
    const result: Route[] = await this._routeService.bulkUpdate(
      routes.map((item) => item.id),
      request.inputs.all(),
    );
  ...
  async moveTo(request: Request): Promise<ResponseBuilder> {
    ...
    const fromRule: Route = await db
      ...
      .innerJoin('table.firewall', 'firewall')
      .innerJoin('firewall.fwCloud', 'fwcloud')
      .where('route.id = :id', { id: request.inputs.get<number>('fromId') })
      .andWhere('table.id = :table', { table: this._routingTable.id })
      .andWhere('firewall.id = :firewall', { firewall: this._firewall.id })
      .andWhere('firewall.fwCloudId = :fwcloud', { fwcloud: this._fwCloud.id })
      .getOneOrFail();
    ...
    const toRule: Route = await db
      ...
      .innerJoin('table.firewall', 'firewall')
      .innerJoin('firewall.fwCloud', 'fwcloud')
      .where('route.id = :id', { id: request.inputs.get<number>('toId') })
      .andWhere('table.id = :table', { table: this._routingTable.id })
      .andWhere('firewall.id = :firewall', { firewall: this._firewall.id })
      .andWhere('firewall.fwCloudId = :fwcloud', { fwcloud: this._fwCloud.id })
      .getOneOrFail();

    const result: Route[] = await this._routeService.moveTo(
      fromRule.id,
      toRule.id,
      request.inputs.all(),
    );
  ...
  async moveToGateway(request: Request): Promise<ResponseBuilder> {
    ...
    const result: Route[] = await this._routeService.moveToGateway(
      fromRule.id,
      toRule.id,
      request.inputs.all(),
    );
  ...
  async moveInterface(request: Request): Promise<ResponseBuilder> {
    ...
    const result: Route[] = await this._routeService.moveInterface(
      fromRule.id,
      toRule.id,
      request.inputs.all(),
    );
