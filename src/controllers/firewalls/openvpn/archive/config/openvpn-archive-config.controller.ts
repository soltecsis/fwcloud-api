...
export class OpenVPNArchiveConfigController extends Controller {
  ...
  async make() {
    this._openvpnService = await app().getService<OpenVPNService>(
      OpenVPNService.name,
    );
  }
  ...
  public async show(): Promise<ResponseBuilder> {
    const config: OpenVPNUpdateableConfig =
      this._openvpnService.getCustomizedConfig();
  ...
  public async update(request: Request): Promise<ResponseBuilder> {
    await this._openvpnService.updateArchiveConfig({
      history: request.body as unknown as {
        archive_days: number;
