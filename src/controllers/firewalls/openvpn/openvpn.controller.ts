...
export class OpenVPNController extends Controller {
  ...
  public async make(request: Request): Promise<void> {
    if (request.params.openvpn) {
      this._openvpn = await getRepository(OpenVPN).findOneOrFail(
        parseInt(request.params.openvpn),
      );
    }
  ...
  public async installer(req: Request): Promise<ResponseBuilder> {
    ...
    const exePath: string = await (
      await this._app.getService<OpenVPNService>(OpenVPNService.name)
    ).generateInstaller(
  ...
  public async history(req: Request): Promise<ResponseBuilder> {
    ...
    const options: FindOpenVPNStatusHistoryOptions = this.buildOptions(
      req.query,
    );

    const historyService: OpenVPNStatusHistoryService =
      await app().getService<OpenVPNStatusHistoryService>(
        OpenVPNStatusHistoryService.name,
      );
    const results: FindResponse = await historyService.history(
      openVPN.id,
      options,
    );
  ...
  public async graph(req: Request): Promise<ResponseBuilder> {
    ...
    const options: GraphOpenVPNStatusHistoryOptions = this.buildOptions(
      req.query,
    );
    ...
    const results: GraphDataResponse = await historyService.graph(
      openVPN.id,
      options,
    );
