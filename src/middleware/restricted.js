  ...
  const promise = new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      let result = await queryRunner.query(
        'SELECT cluster from firewall where id=' +
          req.body.firewall +
          ' AND fwcloud=' +
          req.body.fwcloud,
      );

      const data_pr = await queryRunner.query(
        'SELECT count(*) as cont FROM policy_r R inner join firewall F on R.firewall=F.id' +
          ' where fw_apply_to=' +
          req.body.firewall +
          ' AND F.cluster=' +
          result[0].cluster +
      ...
      const data_rr = await queryRunner.query(
        'SELECT count(*) as cont FROM routing_r RR inner join routing_table RT on RR.routing_table = RT.id' +
          ' inner join firewall F on RT.firewall = F.id' +
          ' where RR.fw_apply_to=' +
          req.body.firewall +
          ' AND F.cluster=' +
          result[0].cluster +
          ' AND F.fwcloud=' +
      ...
      const data_r = await queryRunner.query(
        'SELECT count(*) as cont FROM route R inner join routing_table RT on R.routing_table = RT.id' +
          ' inner join firewall F on RT.firewall = F.id' +
          ' where R.fw_apply_to=' +
          req.body.firewall +
          ' AND F.cluster=' +
  ...
  promise
    .then((message) => {
      if (message) {
        const restricted = {
          result: false,
          restrictions: 'FIREWALL WITH RESTRICTIONS APPLY_TO ON RULES',
        };
  ...
  try {
    ...
    if (data.result) {
      ...
      for (let key in data.restrictions) {
        if (key === 'InterfaceInFirewall' || key === 'InterfaceInHost') continue;
