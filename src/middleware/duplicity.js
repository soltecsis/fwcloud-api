...
duplicityCheck.ipobj = (req, res, next) => {
  ...
  let sql;
  if (req.body.type === 5 || req.body.type === 7) {
    // 5: ADDRESS, 7: NETWORK
    ...
    try {
      if (rows.length > 0) {
        if (req.body.type === 5 || req.body.type === 7) {
          ...
          const net1 =
            req.body.netmask[0] === '/'
              ? ip.cidrSubnet(`${req.body.address}${req.body.netmask}`)
              : ip.subnet(req.body.address, req.body.netmask);
          let net2 = {};
          for (row of rows) {
            net2 =
              row.netmask[0] === '/'
            ...
            if (net1.subnetMaskLength === net2.subnetMaskLength) throw fwcError.ALREADY_EXISTS;
          }
          next();
