...
schema.validate = (req) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (
      (req.method === 'POST' && req.url === '/cluster') ||
      (req.method === 'PUT' && req.url === '/cluster')
    ) {
      try {
        if (req.body.clusterData.fwnodes) {
          ...
          for (let i = 0; i < nodes.length; i++) {
            ...
            if (nodes[i].install_user)
              nodes[i].install_user = await pgp.decrypt(nodes[i].install_user);
            if (nodes[i].install_pass)
              nodes[i].install_pass = await pgp.decrypt(nodes[i].install_pass);
            if (nodes[i].install_apikey)
              nodes[i].install_apikey = await pgp.decrypt(nodes[i].install_apikey);
          }
        }
      } catch (error) {
        return reject(fwcError.other(`PGP decrypt: ${error.message}`));
      }
      ...
      var schemaItem = Joi.object().keys({
        ...
        install_communication: Joi.string()
          .regex(/ssh|agent/)
          .default('ssh'),
      ...
      else if (req.url !== '/cluster/cloud/get') return reject(fwcError.BAD_API_CALL);
