...
schema.validate = req => {
	return new Promise(async (resolve, reject) => {
		...
		if (req.method === 'POST' || req.method === 'PUT') {
			if (req.method === 'PUT' && /^\/policy\/prefix\/(openvpn|wireguard|ipsec)\/move$/.test(req.path)) {
				schema = schema.append({ new_rule: sharedSch.id, new_position: sharedSch.rule_position, new_order: sharedSch.u16bits });
			}
			else if (req.method === 'PUT' && /^\/policy\/prefix\/(openvpn|wireguard|ipsec)\/order$/.test(req.path)) {
