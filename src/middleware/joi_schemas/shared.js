...
sharedSchema.comment = Joi.string()
  .allow('')
  .allow(null)
  .regex(/^[\x09-\x0D -~\x80-\xFE]{1,254}$/)
  .optional();

sharedSchema.script_code = Joi.string()
  .allow('')
  .allow(null)
  .regex(/^[\x09-\x0D -~\x80-\xFE]{1,65535}$/)
  .optional();
...
sharedSchema.linux_user = Joi.string().regex(
  /^[a-zA-Z_]([a-zA-Z0-9_-]{0,31}|[a-zA-Z0-9_-]{0,30}\$)$/,
);
sharedSchema.linux_pass = Joi.string().regex(/^[ -~\x80-\xFE]{2,64}$/);

sharedSchema.linux_path = Joi.string().regex(
  /^\/{1}(((\/{1}\.{1})?[a-zA-Z0-9 -_]+\/?)+(\.{1}[a-zA-Z0-9]{2,4})?)$/,
);
...
sharedSchema.policy_type = Joi.number()
  .integer()
  .valid(1, 2, 3, 4, 5, 6, 61, 62, 63, 64, 65, 66, 67, 68, 66, 67, 68);
...
sharedSchema.ipv4_netmask = Joi.string().regex(
...
sharedSchema.dns_name = Joi.string().regex(
  /^(([a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*([A-Za-z0-9]|[A-Za-z0-9][A-Za-z0-9\-]*[A-Za-z0-9])$/,
);

sharedSchema.role = Joi.number().integer().min(1).max(2);

sharedSchema.backup_id = Joi.string().regex(
  /^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])_(2[0-3]|[01][0-9]):[0-5][0-9]:[0-5][0-9]$/,
);
...
sharedSchema.cron_schedule = Joi.string().regex(
