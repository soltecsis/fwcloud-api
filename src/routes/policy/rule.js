...
router.put('/', utilsModel.disableFirewallCompileStatus, async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      policy_rData.type !== 2 &&
      policy_rData.type !== 3 &&
      policy_rData.type !== 61 &&
      policy_rData.type !== 62 &&
      policy_rData.type !== 63
    )
...
router.put('/type/get', async (req, res) => {
  try {
    const policy = await PolicyRule.getPolicyData(
      'grid',
      req.dbCon,
      req.body.fwcloud,
      req.body.firewall,
...
router.put('/get', async (req, res) => {
  try {
    ...
    if (policy && policy.length > 0) res.status(200).json(policy[0]);
...
router.put('/full/get', async (req, res) => {
  try {
    const data = await PolicyRule.getPolicyData(
      'grid',
      req.dbCon,
      req.body.fwcloud,
      req.body.firewall,
...
router.put('/active', utilsModel.disableFirewallCompileStatus, async (req, res) => {
...
router.put('/style', utilsModel.disableFirewallCompileStatus, async (req, res) => {
  const policyRuleRepository = getCustomRepository(PolicyRuleRepository);
...
/* Copy RULES */
router.put('/copy', utilsModel.disableFirewallCompileStatus, async (req, res) => {
  try {
    let pasteOnRuleId = req.body.pasteOnRuleId;
    for (let rule of req.body.rulesIds)
      pasteOnRuleId = await ruleCopy(
        req.dbCon,
        req.body.firewall,
        rule,
        req.body.pasteOffset === 1 ? pasteOnRuleId : req.body.pasteOnRuleId,
        req.body.pasteOffset,
      );
    res.status(204).end();
  } catch (error) {
...
router.put('/move', utilsModel.disableFirewallCompileStatus, async (req, res) => {
  try {
    ...
    for (let rule of req.body.rulesIds)
      if (rule === pasteOnRuleId)
        throw fwcError.other('Paste on rule can not be part of the set of pasted rules.');
...
router.put(
  ['/position/negate', '/position/allow'],
...
async function ruleMove(dbCon, firewall, rule, pasteOnRuleId, pasteOffset) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      else new_order = moveRule.rule_order;
      ...
      if (pasteOffset != 0 && moveRule.idgroup) {
        const policyGroup = await getCustomRepository(PolicyGroupRepository).findOne(
          moveRule.idgroup,
        );
        if (policyGroup) {
