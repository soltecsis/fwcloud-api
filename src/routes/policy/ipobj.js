...
router.post('/', utilsModel.disableFirewallCompileStatus, async (req, res) => {
  ...
  try {
    ...
    if (!(await PolicyRuleToIPObj.checkIpVersion(req))) throw fwcError.IPOBJ_BAD_IP_VERSION;
...
router.put('/move', utilsModel.disableFirewallCompileStatus, async (req, res) => {
  ...
  try {
    ...
    if (psts.content1 === psts.content2) {
      // MOVE BETWEEN POSITIONS WITH THE SAME CONTENT TYPE
    ...
    } else if (psts.content1 === 'I' && psts.content2 === 'O') {
      // MOVE BETWEEN POSITIONS WITH DIFFERENT CONTENT TYPE
      ...
      await PolicyRuleToIPObj.insertPolicy_r__ipobj(policy_r__ipobjData);
      await PolicyRuleToInterface.deletePolicy_r__interface(
        req.dbCon,
        rule,
        interfaceName,
        position,
      ...
      // NOT ALLOWED TO MOVE BETWEEN THESE POSITIONS BECAUSE THE CONTENT TYPE
...
router.put('/order', utilsModel.disableFirewallCompileStatus, (req, res) => {
  ...
  PolicyRuleToIPObj.updatePolicy_r__ipobj_position_order(
    rule,
    ipobj,
    ipobj_g,
    interfaceName,
