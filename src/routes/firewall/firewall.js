...
router.post('/', async (req, res) => {
  var firewallData = {
    ...
    install_communication:
      req.body.install_communication === 'ssh'
        ? FirewallInstallCommunication.SSH
        : FirewallInstallCommunication.Agent,
    ...
    install_apikey:
      req.body.install_apikey !== null ? await utilsModel.encrypt(req.body.install_apikey) : null,
  ...
  try {
    if (!req.body.cluster) {
      ...
      if (
        nodes.length >= app().config.get('limits').nodes &&
        app().config.get('limits').nodes > 0
      ) {
    ...
    await Firewall.updateFWMaster(
      req.session.user_id,
      req.body.fwcloud,
      firewallData.cluster,
      newFirewallId,
      firewallData.fwmaster,
    );

    if (
      (firewallData.cluster > 0 && firewallData.fwmaster === 1) ||
    ...
    if (!firewallData.cluster)
      // Create firewall tree.
      await Tree.insertFwc_Tree_New_firewall(req.body.fwcloud, req.body.node_id, newFirewallId);
    // Create the new firewall node in the NODES node of the cluster.
    else
      await Tree.insertFwc_Tree_New_cluster_firewall(
        req.body.fwcloud,
        firewallData.cluster,
        newFirewallId,
        firewallData.name,
  ...
  try {
    ...
    if (data) {
      ...
      if (data.install_apikey !== null)
        data.install_apikey = await pgp.encrypt(data.install_apikey);
...
router.put('/clone', async (req, res) => {
  try {
    ...
    const dataI = await Interface.cloneFirewallInterfaces(
      req.session.user_id,
      req.body.fwcloud,
      req.body.firewall,
      idNewFirewall,
...
router.put('/restricted', restrictedCheck.firewall, restrictedCheck.firewallApplyTo, (req, res) =>
  res.status(204).end(),
);
...
router.put('/delfromcluster', restrictedCheck.firewallApplyTo, async (req, res) => {
  //CHECK FIREWALL DATA TO DELETE
  try {
    const firewallService = await app().getService(FirewallService.name);
    const data = await firewallService.deleteFirewallFromCluster(
      req.body.cluster,
      req.body.firewall,
      req.body.fwcloud,
      req.session.user_id,
    );
    if (data && data.result) res.status(200).json(data);
    else res.status(204).end();
  } catch (error) {
