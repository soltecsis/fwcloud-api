...
router.post('/', async (req, res) => {
  ...
  try {
    if (
      !(await Tree.verifyNodeInfo(
        node_parent,
        fwcloud,
        host === null || host === undefined ? firewall : host,
      ))
    ) {
      logger().error(
        'Error creating new interface: ' + JSON.stringify(fwcError.BAD_TREE_NODE_TYPE),
      );
    ...
    if (insertId && insertId > 0) {
      if (host) {
        ...
        if (id2 && id2 > 0) await InterfaceIPObj.UpdateHOST(id2);
      }
...
router.put('/fw/del', restrictedCheck.interface, async (req, res) => {
  try {
...
router.put('/host/del', restrictedCheck.interface, async (req, res) => {
  try {
...
router.put('/where', async (req, res) => {
  try {
    const data = await Interface.searchInterfaceUsage(
      req.body.id,
      req.body.type,
      req.body.fwcloud,
      null,
  ...
  try {
    ...
    logger().error(
      'Error getting network interface information: ' +
        Object.prototype.hasOwnProperty(error, 'message')
        ? error.message
