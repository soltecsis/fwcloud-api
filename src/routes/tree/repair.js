  ...
  try {
    if (req.body.type === 'FDF') {
      channel.emit(
        ...
        new ProgressInfoPayload(
          `REPAIRING FIREWALLS/CLUSTERS TREE FOR CLOUD WITH ID: ${req.body.fwcloud}\n`,
          true,
        ),
      );
      ...
      channel.emit(
        'message',
        new ProgressInfoPayload(
          `REPAIRING OBJECTS TREE FOR CLOUD WITH ID: ${req.body.fwcloud}\n`,
          true,
        ),
      );
      ...
      channel.emit(
        'message',
        new ProgressInfoPayload(
          `REPAIRING SERVICES TREE FOR CLOUD WITH ID: ${req.body.fwcloud}\n`,
          true,
        ),
      );
    } else {
    ...
    for (let rootNode of rootNodes) {
      if (rootNode.node_type === 'FDF' && req.body.type === 'FDF') {
        ...
        const openvpn_srv_list = await OpenVPN.getOpenvpnServersByCloud(
          req.dbCon,
          req.body.fwcloud,
        );
