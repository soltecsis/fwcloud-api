 ...
 *      };
 * */
router.post('/', duplicityCheck.ipobj, async (req, res) => {
  ...
  try {
    ...
    //INSERT IN TREE
    const node_id = node_parent
      ? await Tree.insertFwc_TreeOBJ(req, node_parent, node_order, node_type, ipobjData)
      : 0;
 ...
 *      };
 * */
router.put('/', duplicityCheck.ipobj, async (req, res) => {
  ...
  try {
    ...
    if (data && data[0].protocol_number !== null) ipobjData.protocol = data[0].protocol_number;
 ...
 *      };
 */
router.put('/del', restrictedCheck.ipobj, async (req, res) => {
  var fwcloud = req.body.fwcloud;
  var id = req.body.id;
  ...
  try {
    ...
    if (type === 8) await IPObj.deleteHost(req.dbCon, req.body.fwcloud, req.body.id);
    else await IPObj.deleteIpobj(req.dbCon, req.body.fwcloud, req.body.id);
...
router.put('/where', async (req, res) => {
  try {
    const data = await IPObj.searchIpobjUsage(
      req.dbCon,
      req.body.fwcloud,
      req.body.id,
      req.body.type,
    );
    if (data.result) res.status(200).json(data);
