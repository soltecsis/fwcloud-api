...
router.post('/', async (req, res) => {
  try {
    if (req.tree_node.node_type !== 'MRK') throw fwcError.BAD_TREE_NODE_TYPE;
...
router.put('/', async (req, res) => {
  try {
    ...
    if (existsId && existsId !== req.body.mark) throw fwcError.ALREADY_EXISTS;
    ...
    const mark = await db
      ...
      .findOneOrFail(req.body.mark, {
        relations: [
          'policyRules',
          'routingRuleToMarks',
          'routingRuleToMarks.routingRule',
          'routingRuleToMarks.routingRule.routingTable',
        ],
      });
 ...
 */
router.put('/del', restrictedCheck.mark, async (req, res) => {
  try {
