...
router.post('/', (req, res) => {
  ...
  IPObjGroup.insertIpobj_g(ipobj_gData, async (error, data) => {
    ...
    if (data && data.insertId > 0) {
      ...
      //INSERT IN TREE
      try {
        const node_id = await Tree.insertFwc_TreeOBJ(
          req,
          node_parent,
          node_order,
          node_type,
...
router.put('/get', async (req, res) => {
  try {
    ...
    if (data && data.length == 1) res.status(200).json(data[0]);
    else {
...
/* Remove ipobj_g */
router.put('/del', restrictedCheck.ipobj_group, async (req, res) => {
  try {
...
router.put('/addto', async (req, res) => {
  try {
    ...
    if (req.body.node_type === 'OCL') {
      ...
      dataIpobj = await OpenVPNPrefix.getPrefixOpenvpnInfo(
        req.dbCon,
        req.body.fwcloud,
        req.body.prefix,
      );
      ...
      if (dataIpobj.length > 0 && dataIpobj[0].type === 8) {
        if (
          dataIpobj[0].interfaces.length === 0 ||
          dataIpobj[0].interfaces.filter((item) => item.ipobjs.length > 0).length === 0
        ) {
          throw fwcError.IPOBJ_EMPTY_CONTAINER;
        }
      }
...
router.put('/delfrom', async (req, res) => {
  try {
    ...
    if ((await IPObjGroup.countGroupItems(req.dbCon, req.body.ipobj_g)) === 1) {
      const policyRules = await PolicyRuleToIPObj.searchGroupInRule(
        req.body.ipobj_g,
        req.body.fwcloud,
      );

      if (policyRules.length > 0) throw fwcError.IPOBJ_EMPTY_CONTAINER;
      ...
      const routes = await db
        ...
        .innerJoinAndSelect('routeToIPObjGroups.ipObjGroup', 'group', `group.id = :group`, {
          group: req.body.ipobj_g,
        })
        .getCount();

      if (routes > 0) throw fwcError.IPOBJ_EMPTY_CONTAINER;
      ...
      const routingRules = await db
        ...
        .innerJoinAndSelect('routingRuleToIPObjGroups.ipObjGroup', 'group', `group.id = :group`, {
          group: req.body.ipobj_g,
        })
        .getCount();

      if (routingRules > 0) throw fwcError.IPOBJ_EMPTY_CONTAINER;
    }

    if (req.body.obj_type === 311)
      // OPENVPN CLI
      await OpenVPN.removeFromGroup(req);
    else if (req.body.obj_type === 401)
      // OpenVPN PREFIX
      await OpenVPNPrefix.removePrefixFromGroup(req);
    else await IPObjToIPObjGroup.deleteIpobj__ipobjg(req.dbCon, req.body.ipobj_g, req.body.ipobj);

    await Tree.deleteFwc_TreeGroupChild(
      req.dbCon,
      req.body.fwcloud,
      req.body.ipobj_g,
      req.body.ipobj,
