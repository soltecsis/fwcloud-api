...
router.post('/', async (req, res) => {
  try {
    ...
    if (await Crt.existsCRT(req.dbCon, req.body.ca, req.body.cn)) throw fwcError.ALREADY_EXISTS;
...
router.put('/exists', async (req, res) => {
  try {
    if (await Crt.existsCRT(req.dbCon, req.body.ca, req.body.cn)) res.status(200).end();
 ...
 * Delete certificate.
 */
router.put('/del', restrictedCheck.crt, async (req, res) => {
  try {
