...
router.post('/', async (req, res) => {
  try {
    ...
    if (req.openvpn.type !== 2) throw fwcError.VPN_NOT_SER;
...
router.put('/info/get', async (req, res) => {
  try {
    const data = await OpenVPNPrefix.getPrefixOpenvpnInfo(
      req.dbCon,
      req.body.fwcloud,
      req.body.prefix,
    );
 ...
 */
router.put('/del', restrictedCheck.openvpn_prefix, async (req, res) => {
  try {
    // Delete prefix.
