...
router.post('/', async (req, res) => {
  try {
    ...
    if (req.crt.type === 1 && !req.body.openvpn)
      throw {
        msg: 'When using client certificates you must indicate the OpenVPN server configuration',
      };
    if (req.crt.type === 2 && req.body.openvpn)
      throw {
        msg: 'When using server certificates you must not indicate the OpenVPN server configuration',
      };
    ...
    if (req.crt.type === 1 && req.crt.ca !== req.openvpn.ca)
      throw {
        msg: 'CRT for a new client OpenVPN configuration must has the same CA that the server OpenVPN configuration to which it belongs',
      };
    ...
    if (req.crt.type === 1 && req.body.firewall !== req.openvpn.firewall)
      throw {
        msg: 'Firewall ID for the new client OpenVPN configuration must match server OpenVPN configuration',
      };
    ...
    let nodeId;
    if (req.tree_node.node_type === 'OPN')
      // This will be an OpenVPN server configuration.
    ...
    else if (req.tree_node.node_type === 'OSR') {
      // This will be an OpenVPN client configuration.
      ...
      await OpenVPNPrefix.updateOpenvpnClientPrefixesFWStatus(
        req.dbCon,
        req.body.fwcloud,
        newOpenvpn,
      );
...
router.put('/info/get', async (req, res) => {
  try {
    const data = await OpenVPN.getOpenvpnInfo(
      req.dbCon,
      req.body.fwcloud,
      req.body.openvpn,
      req.openvpn.type,
 ...
 */
router.put('/del', restrictedCheck.openvpn, async (req, res) => {
  try {
...
router.put('/where', async (req, res) => {
  try {
    const data = await OpenVPN.searchOpenvpnUsage(
      req.dbCon,
      req.body.fwcloud,
      req.body.openvpn,
      true,
    );
    if (data.result > 0) res.status(200).json(data);
...
router.put('/install', async (req, res, next) => {
  try {
    ...
    const firewall = await db
      ...
      .findOneOrFail(req.body.firewall);
    ...
    if (crt.type === 1) {
      ...
      const openvpn_opt = await OpenVPN.getOptData(
        req.dbCon,
        req.openvpn.openvpn,
        'client-config-dir',
      );
      if (!openvpn_opt) throw fwcError.VPN_NOT_FOUND_CFGDIR;
      await communication.installOpenVPNClientConfigs(
        openvpn_opt.arg,
        [
          {
            content: cfgDump.ccd,
    ...
    logger().error(
      'Error installing openvpn: ' + Object.prototype.hasOwnProperty(error, 'message')
        ? error.message
...
router.put('/uninstall', async (req, res, next) => {
  try {
    ...
    logger().error(
      'Error uninstalling openvpn: ' + Object.prototype.hasOwnProperty(error, 'message')
        ? error.message
        : JSON.stringify(error),
    );
...
router.put('/ccdsync', async (req, res, next) => {
  try {
    ...
    let lastClusterNodeId = cluster
      ? await Firewall.getLastClusterNodeId(req.dbCon, cluster)
      : null;
    ...
    if (toBeInstalled.length > 0) {
      const toBeInstalledOpenVPNs = await db
        .getSource()
        .manager.getRepository(OpenVPN)
        .createQueryBuilder('openvpn')
        .innerJoinAndSelect('openvpn.crt', 'crt')
    ...
    logger().error(
      'Error openvpn ccd sync: ' + Object.prototype.hasOwnProperty(error, 'message')
        ? error.message
        : JSON.stringify(error),
    );
...
router.put('/status/get', async (req, res, next) => {
  try {
        ...
        host: Object.prototype.hasOwnProperty.call(req.body, 'host')
          ...
          : (
              await db
                ...
                .findOneOrFail(firewall.install_ipobj)
            ).address,
    ...
    logger().error(
      'Error getting openvpn log file: ' + Object.prototype.hasOwnProperty(error, 'message')
        ? error.message
        : JSON.stringify(error),
    );
