...
accessCtrl.check = async (req, res, next) => {
  ...
  if (req.url.substring(0, 5) === '/user' || req.url.substring(0, 9) === '/customer') {
    ...
    if (req.url === '/user/changepass') return next();

    // Any user can end the session.
    if (req.url === '/user/logout') return next();

    // All other customer and user changes are only allowed to administrator users.
    if (await User.isLoggedUserAdmin(req)) return next();
  ...
  try {
    ...
    if (req.body.fwcloud && !(await checkFwCloudAccess(req))) throw fwcError.ACC_FWCLOUD;
    ...
    if (req.body.firewall) {
      ...
      if (!(await Firewall.getFirewallAccess(accessData))) throw fwcError.ACC_FIREWALL;
    }
    ...
    if (req.body.cluster || (req.body.clusterData && req.body.clusterData.cluster)) {
      if (
        !(await checkClusterAccess(
          req,
          req.body.cluster ? req.body.cluster : req.body.clusterData.cluster,
        ))
      )
    ...
    if (req.body.node_id) {
      if (!(await checkTreeNodeAccess(req))) throw fwcError.ACC_TREE_NODE;
    }

    // Check access to the CA indicated in req.body.ca.
    if (req.body.ca) {
      if (!(await checkCAAccess(req))) throw fwcError.ACC_CA;
    }

    // Check access to the crt indicated in req.body.crt.
    if (req.body.crt) {
      if (!(await checkCrtAccess(req))) throw fwcError.ACC_CRT;
    }

    // Check access to the openvpn indicated in req.body.openvpn.
    if (req.body.openvpn) {
      if (!(await checkOpenVPNAccess(req))) throw fwcError.ACC_OPENVPN;
    }

    // Check access to the CRT prefix indicated in req.body.prefix.
    if (req.body.prefix) {
      if (!(await checkPrefixAccess(req))) throw fwcError.ACC_CRT_PREFIX;
    }

    // Check access to the rule indicated by req.body.rule o req.body.new_rule.
    if (req.body.rule) {
      if (!(await checkPolicyRuleAccess(req, req.body.rule))) throw fwcError.ACC_POLICY_RULE;
    }
    if (req.body.new_rule) {
      if (!(await checkPolicyRuleAccess(req, req.body.new_rule))) throw fwcError.ACC_POLICY_RULE;
    }
    if (req.body.rulesIds) {
      for (let rule of req.body.rulesIds) {
        if (!(await checkPolicyRuleAccess(req, rule))) throw fwcError.ACC_POLICY_RULE;
      }
    }

    // Check access to the iptables mark indicated by req.body.mark parameter.
    if (req.body.mark) {
      if (!(await checkIptablesMarkAccess(req))) throw fwcError.ACC_IPTABLES_MARK;
    }
...
function checkClusterAccess(req, cluster) {
  return new Promise((resolve, reject) => {
    req.dbCon.query(
      `select id FROM cluster WHERE id=${cluster} and fwcloud=${req.body.fwcloud}`,
      (error, result) => {
...
function checkIptablesMarkAccess(req) {
  return new Promise((resolve, reject) => {
    req.dbCon.query(
      `select id from mark where id=${req.body.mark} and fwcloud=${req.body.fwcloud}`,
      (error, result) => {
        if (error) return reject(error);
        if (result.length !== 1) return resolve(false);
...
function checkPrefixAccess(req) {
  return new Promise((resolve, reject) => {
    ...
    if (item[1] === 'vpn' && item[2] === 'pki' && item[3] === 'prefix') {
      ...
      (item[1] === 'vpn' && item[2] === 'openvpn' && item[3] === 'prefix') ||
      (item[1] === 'policy' && item[2] === 'prefix') ||
