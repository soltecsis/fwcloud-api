...
export class Firewall extends Model {
  ...
  async getCommunication(
    ...
    if (this.install_communication === FirewallInstallCommunication.SSH) {
      return new SSHCommunication({
        host: (await getRepository(IPObj).findOneOrFail(this.install_ipobj))
          .address,
  ...
  public async hasMarkedRules(): Promise<boolean> {
    return (
      (
        await getRepository(PolicyRule).find({
          where: {
            firewallId: this.id,
            markId: Not(IsNull()),
          },
  ...
  public static async updateFirewallStatusIPOBJ(
    ...
    const query = getRepository(Firewall)
    ...
    const groupContainers: IPObjGroup[] = await getRepository(IPObjGroup)
    ...
    const interfacesUsingAddress: Interface[] = await getRepository(Interface)
  ...
  public static async updateFirewallStatusInterface(
    ...
    const interfaces: Interface[] = await getRepository(Interface)
    ...
    const hosts: IPObj[] = await getRepository(IPObj)
