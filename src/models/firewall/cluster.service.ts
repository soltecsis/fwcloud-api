            ...
            db.getQuery().query(sql, async (error, fws) => {
                ...
                db.getQuery().query(sql, async (error, cluster) => {
                    ...
                    try {
                        ...
                        if (cluster.length > 0)
                            await Tree.deleteFwc_TreeFullNode({ id: cluster[0].idnode, fwcloud: fwcloudId, iduser: userId });
                    } catch (error) { 
                        return reject(error) 
                    }

                    db.getQuery().query(`DELETE FROM ${Cluster._getTableName()} WHERE id=${cluster[0].id}`, (error, result) => {
                        if (error) return reject(error);
                        resolve();
                    });
                });
