...
export class RoutingRuleRepository extends Repository<RoutingRule> {
   ...
   */
  findManyInPath(
    path: IFindManyRoutingRulePath,
    options?: FindManyOptions<RoutingRule>,
  ): Promise<RoutingRule[]> {
  ...
  async remove(
    ...
    const entityArray: RoutingRule[] = Array.isArray(entityOrEntities)
      ? entityOrEntities
      : [entityOrEntities];
  ...
  protected async refreshOrders(firewallId: number): Promise<void> {
    const firewall: Firewall =
      await getRepository(Firewall).findOneOrFail(firewallId);
  ...
  protected getFindInPathOptions(
    path: Partial<IFindOneRoutingRulePath>,
    options: FindOneOptions<RoutingRule> | FindManyOptions<RoutingRule> = {},
  ): FindOneOptions<RoutingRule> | FindManyOptions<RoutingRule> {
