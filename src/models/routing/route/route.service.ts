...
export class RouteService extends Service {
    ...
    async update(id: number, data: IUpdateRoute): Promise<Route> {
        ...
        if (data.openVPNIds) {
            ...
            route.routeToOpenVPNs = data.openVPNIds.map(item => ({
                routeId: route.id,
                openVPNId: item.id,
                order: item.order
            } as RouteToOpenVPN));
        ...
        if (data.openVPNPrefixIds) {
            ...
            route.routeToOpenVPNPrefixes = data.openVPNPrefixIds.map(item => ({
                routeId: route.id,
            ...
            if (ipObjGroup.type !== 20) {
                ...
                for(const ipObjToIPObjGroup of ipObjGroup.ipObjToIPObjGroups) {
                    if (ipObjToIPObjGroup.ipObj.ipObjTypeId === 8) { // 8 = HOST
                        const addrs: any = await Interface.getHostAddr(db.getQuery(), ipObjToIPObjGroup.ipObj.id);
                        if (addrs.length > 0 ) {
                            valid = true;
                        }
                    }
