...
export class RoutingTableService extends Service {
  ...
  async create(data: ICreateRoutingTable): Promise<RoutingTable> {
    ...
    const routingTable: RoutingTable = await this._repository.findOne(
      result.id,
    );
  ...
  async update(id: number, data: IUpdateRoutingTable): Promise<RoutingTable> {
    const table: RoutingTable = await this._repository.preload(
  ...
  protected async validateRoutingTableNumber(
    ...
    const tablesWithSameNumber: RoutingTable[] = await getRepository(
      RoutingTable,
    ).find({
      where: {
