...
export class OpenVPNStatusHistoryService extends Service {
  ...
  async create(
    ...
    const timestamps: number[] = [
      ...new Set(data.map((item) => item.timestampInSeconds)),
    ].sort((a, b) => (a < b ? -1 : 1));
    ...
    for (const timestamp of timestamps) {
      ...
      const persistedBatch = await getRepository(OpenVPNStatusHistory).save(
      ...
      lastTimestampedBatch = await getRepository(
        OpenVPNStatusHistory,
      ).findByIds(persistedBatch.map((item) => item.id));
  ...
  async history(
    ...
    const results: OpenVPNStatusHistory[] = await this.find(
      openVpnServerId,
      options,
    );
    ...
    for (const name of names) {
      const entries: OpenVPNStatusHistory[] = results.filter(
        (item) => item.name === name,
      );
  ...
  async graph(
    ...
    // IMPORTANT! timestamps must be ordered from lower to higher in order to detect disconnection correctly
    const timestamps: number[] = [
      ...new Set(results.map((item) => item.timestampInSeconds)),
    ].sort((a, b) => (a < b ? -1 : 1));
    ...
    const response: GraphDataResponse = timestamps.map((timestampInSeconds) => {
      ...
      const bytesReceivedSent: [number, number] = records.reduce<
        [number, number]
      >(
        (bytes: [number, number], item: OpenVPNStatusHistory) => {
          return [
            bytes[0] + parseInt(item.bytesReceived),
            bytes[1] + parseInt(item.bytesSent),
          ];
        },
        [0, 0],
      );
  ...
  protected limitGraphPoints(
    ...
    while (data.length > 0) {
      ...
      result.push({
        ...
        timestamp:
          group[0].timestamp +
          (group[group.length - 1].timestamp - group[0].timestamp) / 2,
    ...
    for (const previous of previousTimestampedBatch.filter(
      ...
      if (matchIndex < 0) {
        ...
        if (previous.address !== newTimestampedBatch[matchIndex].address) {
          previous.disconnectedAtTimestampInSeconds =
            previous.timestampInSeconds;
          await getRepository(OpenVPNStatusHistory).save(previous);
        }
      }
    }
