...
export class OpenVPNService extends Service {
  ...
  public async generateInstaller(
    ...
    try {
      ...
      configData = (
        (await OpenVPN.dumpCfg(db.getQuery(), fwCloudId, openVPNId)) as any
      ).cfg;
    } catch (e) {
      throw new Error(
      ...
      return await tryAcquire(this._archiveMutex).runExclusive(() => {
        return new Promise<number>(async (resolve, reject) => {
          ...
          const expirationInSeconds: number =
            this._config.history.archive_days * 24 * 60 * 60;
          // If there isn't any row expired, then do nothing
          if (
            (await getExpiredStatusHistoryQuery(expirationInSeconds)
              .limit(1)
              .getCount()) === 0
          ) {
            eventEmitter.emit(
          ...
          while (true) {
            const history: OpenVPNStatusHistory[] =
              await getExpiredStatusHistoryQuery(expirationInSeconds)
                .limit(2000)
                .getMany();
            if (history.length <= 0) {
            ...
            const columns: ColumnMetadataArgs[] =
              ...
              (resolve, reject) => {
                fs.writeFile(
                  ...
                  async (err) => {
                    ...
                    try {
                      await getRepository(OpenVPNStatusHistory).delete(
      ...
      try {
        ...
        fs.readdirSync(this._config.history.data_dir).filter((dirent) => {
          if (fs.existsSync(path.join(this._config.history.data_dir, dirent))) {
            fs.readdirSync(
              path.join(this._config.history.data_dir, dirent),
            ).filter((subDirent) => {
              if (
                fs.existsSync(
                  path.join(this._config.history.data_dir, dirent, subDirent),
                )
              ) {
                fs.readdirSync(
                  path.join(this._config.history.data_dir, dirent, subDirent),
        ...
        const filesToRemove = allFiles
          .filter((fileName) => {
            ...
            //Ignore otherwise
            return new RegExp(/openvpn_status_history-[0-9]{8}\.sql\.zip/).test(
              fileName.file,
            );
          })
          .filter((fileName) => {
            ...
            limit.setDate(
              limit.getDate() - this._config.history.retention_days,
            );
            return date < limit;
          });
        filesToRemove.forEach((fileName) =>
          fs.unlinkSync(path.join(fileName.path, fileName.file)),
        );
   ...
   * @returns
   */
  protected getDateFromArchiveFilename(filename: {
    path: string;
    file: string;
  }): Date {
