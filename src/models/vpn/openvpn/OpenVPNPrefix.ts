...
export class OpenVPNPrefix extends Model {
  ...
  public static existsPrefix(dbCon: Query, openvpn: number, name: string): Promise<boolean> {
  ...
  public static getPrefixes(
    ...
    return new Promise((resolve, reject) => {
      dbCon.query(
        ...
        (error: Error, result: Array<{ id: number; name: string }>) => {
          if (error) return reject(error);
          resolve(result);
        },
      );
  ...
  public static getOpenvpnClientPrefixes(
    ...
    return new Promise((resolve, reject) => {
      ...
      const sql = `select P.id,P.name,CRT.cn from ${tableName} P
                ...
                where V2.id=${openvpn}`;
      dbCon.query(sql, (error: Error, result: Array<{ id: number; name: string; cn: string }>) => {
        if (error) return reject(error);
  ...
  public static getPrefixOpenvpnInfo(dbCon: Query, fwcloud: number, prefix: number) {
    return new Promise((resolve, reject) => {
  ...
  public static fillPrefixNodeOpenVPN(
    ...
    return new Promise((resolve, reject) => {
      ...
      dbCon.query(sql, async (error: Error, result: Array<{ id: number; sufix: string }>) => {
        if (error) return reject(error);
  ...
  public static searchPrefixInRule(
    dbCon: Query,
    fwcloud: number,
    prefix: number,
  ): Promise<Array<PolicyRuleToOpenVPNPrefix & { firewall_id: number; firewall_name: string }>> {
    return new Promise((resolve, reject) => {
      ...
      dbCon.query(
        ...
        (
          error,
          rows: Array<PolicyRuleToOpenVPNPrefix & { firewall_id: number; firewall_name: string }>,
        ) => {
          if (error) return reject(error);
          resolve(rows);
        },
  ...
  public static searchPrefixInGroup(
    dbCon: Query,
    fwcloud: number,
    prefix: number,
  ): Promise<
  ...
  public static async searchPrefixInRoute(
    ...
    return await db
      ...
      .innerJoin('routeToOpenVPNPrefixes.openVPNPrefix', 'prefix', 'prefix.id = :prefix', {
        prefix: prefix,
      })
  ...
  public static async searchPrefixInRoutingRule(
    ...
    return await db
      ...
      .innerJoin('routingRuleToOpenVPNPrefixes.openVPNPrefix', 'prefix', 'prefix.id = :prefix', {
        prefix: prefix,
      })
