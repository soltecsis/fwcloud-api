...
export class OpenVPN extends Model {
  ...
  @OneToMany(
    ...
    (policyRuleToOpenVPN) => policyRuleToOpenVPN.openVPN,
  )
  ...
  public static delCfg(dbCon, fwcloud, openvpn): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      dbCon.query(sql, (error, ipobj_list) => {
        ...
        dbCon.query(
          `delete from openvpn_opt where openvpn=${openvpn}`,
          (error, result) => {
            if (error) return reject(error);

            dbCon.query(
              `delete from openvpn_prefix where openvpn=${openvpn}`,
              (error, result) => {
                if (error) return reject(error);

                dbCon.query(
                  `delete from ${tableName} where id=${openvpn}`,
                  async (error, result) => {
                    if (error) return reject(error);
  ...
  public static updateOpenvpnInstallDate(dbCon, openvpn) {
    return new Promise((resolve, reject) => {
      dbCon.query(
        `UPDATE openvpn SET installed_at=NOW() WHERE id=${openvpn}`,
        (error, result) => {
          if (error) return reject(error);
          resolve({ result: true });
        },
    ...
    return new Promise((resolve, reject) => {
      ...
      req.dbCon.query(sql, (error, result) => {
        ...
        req.dbCon.query(sql, (error, result) => {
          ...
          let found;
          for (
            freeIPLong = net.firstLong;
            freeIPLong <= net.lastLong;
            freeIPLong++
          ) {
            found = 0;
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        /* Verify that the OpenVPN configuration is not used in any
                ...
                */
        search.restrictions.OpenvpnInRule =
          await PolicyRuleToOpenVPN.searchOpenvpnInRule(
            dbCon,
            fwcloud,
            openvpn,
          );
        search.restrictions.OpenvpnInGroup =
          await PolicyRuleToOpenVPN.searchOpenvpnInGroup(
            dbCon,
            fwcloud,
            openvpn,
          );
        search.restrictions.LastOpenvpnInPrefixInRule =
          await PolicyRuleToOpenVPN.searchLastOpenvpnInPrefixInRule(
            dbCon,
            fwcloud,
            openvpn,
          );
        search.restrictions.LastOpenvpnInPrefixInGroup =
          await PolicyRuleToOpenVPN.searchLastOpenvpnInPrefixInGroup(
            dbCon,
            fwcloud,
            openvpn,
          );

        search.restrictions.OpenVPNInRoute = await this.searchOpenVPNInRoute(
          fwcloud,
          openvpn,
        );
        search.restrictions.OpenVPNInGroupInRoute =
          await this.searchOpenVPNInGroupInRoute(fwcloud, openvpn);
        search.restrictions.OpenVPNInRoutingRule =
          await this.searchOpenVPNInRoutingRule(fwcloud, openvpn);
        ...
        if (extendedSearch) {
          ...
          for (let i = 0; i < search.restrictions.OpenvpnInGroup.length; i++) {
            ...
            search.restrictions.OpenvpnInGroupInRule.push(
          ...
          const prefixes = await OpenVPNPrefix.getOpenvpnClientPrefixes(
            dbCon,
            openvpn,
          );
          ...
          for (let i = 0; i < prefixes.length; i++) {
            ...
            search.restrictions.OpenvpnInPrefixInRule.push(
      ...
      req.dbCon.query(sql, async (error, result) => {
        ...
        try {
          for (const openvpn of result) {
            ...
            if (data.result) {
              answer.restrictions.OpenvpnInRule =
                answer.restrictions.OpenvpnInRule.concat(
                  data.restrictions.OpenvpnInRule,
                );
              answer.restrictions.OpenVPNInRoute =
                answer.restrictions.OpenVPNInRoute.concat(
              ...
              answer.restrictions.OpenvpnInGroup =
                answer.restrictions.OpenvpnInGroup.concat(
          ...
          answer.restrictions.OpenvpnInRule =
            answer.restrictions.OpenvpnInRule.filter(
              (item) => item.firewall_id != req.body.firewall,
            );
          answer.restrictions.OpenVPNInRoute =
            answer.restrictions.OpenVPNInRoute.filter(
              (item) => item.firewall_id != req.body.firewall,
            );
          answer.restrictions.OpenVPNInRoutingRule =
