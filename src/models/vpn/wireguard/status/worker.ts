...
import { WireGuardHistoryRecord } from '../../../../communications/communication';
...
import {
  CreateWireGuardStatusHistoryData,
  WireGuardStatusHistoryService,
} from './wireguard-status-history.service';

async function iterate(application: Application): Promise<void> {
  try {
    const service: WireGuardStatusHistoryService = await application.getService(
      WireGuardStatusHistoryService.name,
    );

    // List of all WireGuard servers with which we have to communicate.
    const openvpns: WireGuard[] = await db
      .getSource()
    ...
    for (const openvpn of openvpns) {
      try {
        ...
        let entries: CreateWireGuardStatusHistoryData[] = [];
        for (const firewall of firewalls) {
          const communication: AgentCommunication =
            (await firewall.getCommunication()) as AgentCommunication;

          const statusOption: WireGuardOption = (await WireGuard.getOptData(
            db.getQuery(),
            openvpn.id,
            'status',
          )) as WireGuardOption;
          ...
          if (statusOption) {
            const data: WireGuardHistoryRecord[] = await communication.getWireGuardHistoryFile(
              statusOption.arg,
            );
        ...
        application.logger().error(`WorkerError: WireGuard ${openvpn.id} failed: ${error.message}`);
      }
    }
  } catch (error) {
