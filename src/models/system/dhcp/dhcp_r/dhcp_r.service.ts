...
export class DHCPRuleService extends Service {
  ...
  async store(data: ICreateDHCPRule): Promise<DHCPRule> {
    ...
    if (data.group) {
      dhcpRuleData.group = (await getRepository(DHCPGroup).findOneOrFail(
        data.group,
      )) as DHCPGroup;
    }
    if (data.networkId) {
      dhcpRuleData.network = (await getRepository(IPObj).findOneOrFail(
        data.networkId,
      )) as IPObj;
    }
    if (data.rangeId) {
      dhcpRuleData.range = (await getRepository(IPObj).findOneOrFail(
        data.rangeId,
      )) as IPObj;
    }
    if (data.routerId) {
      dhcpRuleData.router = (await getRepository(IPObj).findOneOrFail(
        data.routerId,
      )) as IPObj;
    }
    if (data.interfaceId) {
    ...
    if (data.firewallId) {
      dhcpRuleData.firewall = (await getRepository(Firewall).findOneOrFail(
        data.firewallId,
      )) as Firewall;
    }
  ...
  async copy(
    ...
    const lastRule: DHCPRule = await this._repository.getLastDHCPRuleInFirewall(
      dhcp_rs[0].firewallId,
    );
  ...
  async moveFrom(
    ...
    if (data.ipObjId !== undefined) {
      const index: number = fromRule.dhcpRuleToIPObjs.findIndex(
  ...
  async update(id: number, data: Partial<ICreateDHCPRule>): Promise<DHCPRule> {
    ...
    if (data.group !== undefined) {
      ...
      dhcpRule.group = data.group
        ? await getRepository(DHCPGroup).findOne(data.group)
        : null;
    } else if (data.ipObjIds) {
      await this.validateUpdateIpObjIds(dhcpRule.firewall, data);
      dhcpRule.dhcpRuleToIPObjs = data.ipObjIds.map(
        (item) =>
          ({
