...
export class KeepalivedRuleService extends Service {
  ...
  async store(data: ICreateKeepalivedRule): Promise<KeepalivedRule> {
    ...
    if (data.group) {
      keepalivedRuleData.group = (await getRepository(
        KeepalivedGroup,
      ).findOneOrFail(data.group)) as KeepalivedGroup;
    }
    if (data.interfaceId) {
    ...
    if (data.masterNodeId) {
      keepalivedRuleData.masterNode = (await getRepository(
        Firewall,
      ).findOneOrFail(data.masterNodeId)) as Firewall;
    }
    if (data.firewallId) {
      keepalivedRuleData.firewall = (await getRepository(
        Firewall,
      ).findOneOrFail(data.firewallId)) as Firewall;
    }
    ...
    if (data.virtualIpsIds && data.virtualIpsIds.length > 0) {
      ...
      const hasMatchingIpVersion = persisted.virtualIps.some(
        async (virtualIp) =>
          (await getRepository(IPObj).findOneOrFail(virtualIp.ipObj))
            .ip_version ===
          (
            await getRepository(IPObj).findOneOrFail(
              persisted.virtualIps[0].ipObj,
            )
          ).ip_version,
      );
  ...
  async update(
    ...
    if (data.group !== undefined) {
      keepalivedRule.group = data.group
        ? await getRepository(KeepalivedGroup).findOne(data.group)
        : null;
      ...
      const hasMatchingIpVersion = keepalivedRule.virtualIps.some(
        async (virtualIp) =>
          ...
          (
            await getRepository(IPObj).findOneOrFail(
              keepalivedRule.virtualIps[0].ipObj,
            )
          ).ip_version,
      );
      if (!hasMatchingIpVersion) {
