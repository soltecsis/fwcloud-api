...
export type PolicyRuleData = PolicyRule & {
  group_name?: string;
  group_style?: string;
...
export class PolicyRule extends Model {
  ...
  public static getPolicy_rs(
    idfirewall: number,
    idgroup: string,
    callback: (error: Error | null, rows: Array<PolicyRule> | null) => void,
  ) {
    db.get((error, connection) => {
      ...
      const sql =
        ...
        whereGroup +
        ' ORDER BY rule_order';
      connection.query(sql, (error, rows: Array<PolicyRule>) => {
        if (error) callback(error, null);
  ...
  public static getPolicyData(
    ...
    return new Promise((resolve, reject) => {
      ...
      dbCon.query(sql, async (error, rulesData: Array<PolicyRuleData>) => {
        if (error) return reject(error);
        ...
        try {
          ...
          const positions: PositionNode[] = await PolicyPosition.getRulePositions(
            dbCon,
            fwcloud,
            rulesData[0].firewallId,
            rulesData[0].id,
            rulesData[0].policyTypeId,
          );
  ...
  public static getPolicy_r(
    dbCon: Query,
    firewall: number,
    rule: number,
  ): Promise<PolicyRule & { max_order: number; min_order: number }> {
    return new Promise((resolve, reject) => {
      ...
      dbCon.query(
        sql,
        (error, result: Array<PolicyRule & { max_order: number; min_order: number }>) => {
          if (error) return reject(error);
  ...
  //Get policy_r by  id
  public static getPolicy_r_id(
    id: number,
    callback: (error: Error | null, row: Array<PolicyRule & { fwcloud: number }>) => void,
  ) {
    db.get((error, connection) => {
      if (error) callback(error, null);
      ...
      connection.query(sql, (error, row: Array<PolicyRule & { fwcloud: number }>) => {
        if (error) {
          logger().debug(error);
          callback(error, null);
  ...
  public static getPolicyRuleType(
    dbCon: Query,
    fwcloud: number,
    firewall: number,
    rule: number,
    ...
    return new Promise((resolve, reject) => {
      ...
      dbCon.query(sql, (error, result: Array<{ type: number }>) => {
        if (error) return reject(error);
  ...
  //Get rule type for a rule
  public static getPolicyRuleIPversion(
    dbCon: Query,
    fwcloud: number,
    firewall: number,
    rule: number,
  ...
  public static getLastRuleOrder(dbCon: Query, firewall: number, type: number): Promise<number> {
    return new Promise((resolve, reject) => {
      const sql = `SELECT rule_order FROM ${tableName} 
				WHERE firewall=${firewall} AND type=${type} ORDER BY rule_order desc limit 1`;

      dbCon.query(sql, (error, result: Array<{ rule_order: number }>) => {
        if (error) return reject(error);
  ...
  public static getPolicy_r_DestGroup(
    ...
    callback: (
      error: Error | null,
      row: Array<{ id: number; idgroup: number; rule_order: number }> | null,
    ) => void,
  ) {
    db.get((error, connection) => {
      let nextRuleStr: string;
      ...
      connection.query(
        sql,
        (error, row: Array<{ id: number; idgroup: number; rule_order: number }>) => {
          if (error) {
            logger().debug(error);
            callback(error, null);
          } else callback(null, row);
  ...
  public static getPolicy_rName(
    idfirewall: number,
    idgroup: string,
    name: number,
    callback: (error: Error | null, row: Array<PolicyRule> | null) => void,
  ) {
    db.get((error, connection) => {
      if (error) callback(error, null);
      const namesql = '%' + name + '%';
      ...
      const sql =
        ...
        whereGroup;
      logger().debug(sql);
      connection.query(sql, (error, row: Array<PolicyRule>) => {
        if (error) callback(error, null);
        else callback(null, row);
      });
    });
  ...
  public static async insertDefaultPolicy(
    fwId: number,
    loInterfaceId: number,
    options: any,
  ): Promise<void> {
    const policy_rData = {
      id: null,
  ...
  public static cloneFirewallPolicy(
    ...
    return new Promise((resolve, reject) => {
      ...
      dbCon.query(sql, async (error, rows: Array<{ newFirewall: number } & PolicyRule>) => {
        if (error) return reject(error);
  ...
  public static async clonePolicy(rowData: { newFirewall: number } & PolicyRule): Promise<void> {
    const dbCon = db.getQuery();

    //CREATE NEW POLICY
    const policy_rData = {
      id: null,
      idgroup: rowData.policyGroupId,
    ...
    const newRule = await this.insertPolicy_r(policy_rData);
    await this.clonePolicyIpobj(dbCon, rowData.newFirewall, rowData.id, newRule);
    await this.clonePolicyInterface(dbCon, rowData.firewallId, rowData.id, newRule);
  ...
  public static clonePolicyIpobj(
    ...
    return new Promise((resolve, reject) => {
      ...
      dbCon.query(
        ...
        async (
          error,
          rows: Array<{ newfirewall: number; newrule: number } & PolicyRuleToIPObj>,
        ) => {
          if (error) return reject(error);
          ...
          if (this.clon_data) {
            ...
            for (let i = 0; i < rows.length; i++) {
              for (const item of this.clon_data) {
                if (rows[i].ipObjId === -1 && rows[i].interfaceId !== -1) {
                  // Replace interfaces IDs with interfaces IDs of the cloned firewall.
                  if (rows[i].interfaceId === item.id_org) {
                    rows[i].interfaceId = item.id_clon;
                    break;
                  }
                } else {
                  ...
                  for (const addr of item.addr) {
                    if (rows[i].ipObjId === addr.id_org) {
                      rows[i].ipObjId = addr.id_clon;
                      found = 1;
                      break;
                    }
                  }
  ...
  public static clonePolicyInterface(
    ...
    return new Promise((resolve, reject) => {
      ...
      dbCon.query(
        ...
        async (
          error,
          rowsI: Array<{ newrule: number; newInterface: number } & PolicyRuleToInterface>,
        ) => {
          if (error) return reject(error);
  ...
  public static makeBeforeRuleOrderGap(
    firewall: number,
    type: number,
    rule: number,
  ): Promise<number> {
    return new Promise((resolve, reject) => {
      db.get((error, connection) => {
        ...
        connection.query(sql, (error, result: Array<{ rule_order: number }>) => {
          if (error) return reject(error);

          let free_rule_order: number;
          let cond = '';
          if (result.length === 1) {
  ...
  public static makeAfterRuleOrderGap(
    firewall: number,
    type: number,
    rule: number,
  ): Promise<number> {
  ...
  public static updateApplyToRules(clusterNew: number, fwNewMaster: number) {
    return new Promise((resolve, reject) => {
      db.get((error, connection) => {
        if (error) return reject(error);
        const sql =
        ...
        connection.query(
          ...
          (
            error,
            rows: Array<{ id: number; fw_apply_to: number; name: string; clusterNew: number }>,
          ) => {
            if (error) return reject(error);
  ...
  public static repointApplyTo(rowData: {
    ...
    return new Promise((resolve, reject) => {
      db.get((error, connection) => {
        ...
        connection.query(sql, (error, rows: Array<{ id: number }>) => {
          if (error) return reject(error);
  ...
  public static async allowEmptyRulePositions(req: RequestData): Promise<void> {
  ...
  //Allow all positions of a rule that are empty.
  public static firewallWithMarkRules(dbCon: Query, firewall: number): Promise<boolean> {
    return new Promise((resolve, reject) => {
      dbCon.query(
  ...
  public static async checkCatchAllRules(dbCon: Query, firewall: number): Promise<void> {
    const policy_rData = {
      id: null,
      idgroup: null,
      firewall: firewall,
    ...
    for (policy_rData.type of [1, 2, 3, 61, 62, 63]) {
      ...
      if (!rule_id) {
        ...
        const rule_data = await this.getPolicy_r(dbCon, firewall, rule_id);
  ...
  public static async createSpecialRule(
    dbCon: Query,
    firewall: number,
    specialRule: SpecialPolicyRules,
  ): Promise<void> {
  ...
  public static async checkSpecialRule(
    dbCon: Query,
    firewall: number,
    options: number,
    specialRule: SpecialPolicyRules,
    ...
    if (options & SpecialRuleToFireWallOptMask.get(specialRule)) {
      ...
      if (await this.existsSpecialRule(dbCon, firewall, specialRule)) return;
  ...
  public static async checkSpecialRules(
    dbCon: Query,
    firewall: number,
    options: number,
  ): Promise<void> {
    ...
    if (!(await this.aloneFirewallOrMasterNode(dbCon, firewall))) return;
