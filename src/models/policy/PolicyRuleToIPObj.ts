...
export interface PolicyRuleToIPObjInRuleData {
  obj_id?: number;
  obj_name?: string;
  obj_type_id?: number;
...
interface PolicyRuleToIPObjData {
  newfirewall?: number;
  newrule?: number;
  ipobj?: number;
  ipobj_g?: number;
...
export class PolicyRuleToIPObj extends Model {
    ...
    return new Promise((resolve, reject) => {
      db.get((error, connection) => {
        ...
        let sql =
          ...
          tableModel +
          ' WHERE rule=' +
          connection.escape(policy_r__ipobjData.rule) +
          ' AND ipobj=' +
          connection.escape(policy_r__ipobjData.ipobj) +
          ' AND ipobj_g=' +
          connection.escape(policy_r__ipobjData.ipobj_g) +
          ' AND interface=' +
          connection.escape(policy_r__ipobjData.interface) +
          ' AND position=' +
          connection.escape(policy_r__ipobjData.position);
        connection.query(sql, (error, rows: Array<{ id_pi: number }>) => {
          if (error) return reject(error);
            ...
            connection.query(sql, (error, rows: Array<{ id_pi: number }>) => {
              ...
              connection.query(sql, (error, rows: Array<{ id_pi: number }>) => {
                ...
                sql =
                  ...
                  ' WHERE IPO.type=8 AND IPO2.type=5 AND PR.rule=' +
                  connection.escape(policy_r__ipobjData.rule) +
                  ' AND PR.position=' +
                  connection.escape(policy_r__ipobjData.position) +
                  ' AND IPO2.id=' +
  ...
  public static isGroupEmpty = (dbCon: Query, group: number): Promise<boolean> => {
    return new Promise((resolve, reject) => {
      ...
      dbCon.query(sql, (error: Error, result: Array<{ id: number }>) => {
        if (error) return reject(error);
  ...
  public static emptyIpobjContainerToObjectPosition(req: RequestData): Promise<boolean> {
    return new Promise((resolve, reject) => {
      ...
      req.dbCon.query(sql, async (error: Error, rows: Array<{ content: string; type: string }>) => {
        if (error) return reject(error);
        ...
        try {
          ...
          if (type === 10 || type === 11) {
            ...
            // 8 = HOST
            const addrs = await Interface.getHostAddr(req.dbCon, req.body.ipobj);
            let n = 0;
            for (const addr of addrs) {
              // Count the amount of interface address with the same IP version of the rule.
              if (addr.ip_version === rule_ip_version) n++;
            ...
            (type === 20 || type === 21) &&
            (await this.isGroupEmpty(req.dbCon, req.body.ipobj_g))
          )
  ...
  public static clonePolicy_r__ipobj(
    ...
    return new Promise((resolve, reject) => {
      ...
      const p_ipobjData = {
        ...
        interfaceId: policy_r__ipobjData.interfaceId,
        position: policy_r__ipobjData.position,
        position_order: policy_r__ipobjData.position_order,
      };
      db.get((error, connection) => {
        if (error) reject(error);
        if (p_ipobjData.interfaceId !== -1) {
          const sqlI =
            'select id from interface where id=' +
            policy_r__ipobjData.interfaceId +
            ' AND firewall= ' +
            newfirewall;
          logger().debug('--------- >>>> SQL INTERFACE OTHER: ', sqlI);
          connection.query(sqlI, async (error, result: Array<{ id: number }>) => {
            if (result && result.length > 0) {
              p_ipobjData.interfaceId = result[0].id;
              await this.cloneInsertPolicy_r__ipobj(p_ipobjData).then(
                (resp: { result: boolean }) => {
          ...
          connection.query(sqlI, async (error, result: Array<{ id: number }>) => {
            if (result && result.length > 0) {
              ...
              await this.cloneInsertPolicy_r__ipobj(p_ipobjData).then((resp) => {
                resolve({ result: resp.result });
              });
            } else {
              await this.cloneInsertPolicy_r__ipobj(p_ipobjData).then((resp) => {
  ...
  public static cloneInsertPolicy_r__ipobj(
    p_ipobjData: PolicyRuleToIPObjData,
          ...
          if (allowed) {
            db.get((error, connection) => {
              ...
              const sql =
                ...
                ',' +
                'interface = ' +
                connection.escape(policy_r__ipobjData.interfaceId) +
                ',' +
                'position = ' +
                connection.escape(policy_r__ipobjData.position) +
                ',' +
                'position_order = ' +
                connection.escape(policy_r__ipobjData.position_order) +
                ', ' +
                ' WHERE rule = ' +
                connection.escape(rule) +
                ' AND ipobj=' +
  ...
  private static async OrderList(
    ...
    return new Promise<any>((resolve, reject) => {
      ...
      let sql_obj = '';
      if (ipobj > 0) sql_obj = ' AND ipobj<>' + ipobj;
      else if (_interface > 0) sql_obj = ' AND interface<>' + _interface;
      else if (ipobj_g > 0) sql_obj = ' AND ipobj_g<>' + ipobj_g;
      db.get((error, connection) => {
        if (error) reject(error);
        const sql =
          'UPDATE ' +
  ...
  private static checkIpobjPosition(
    ...
    _interface: number,
    position: number,
    callback: (err: Error | null, result: boolean | number) => void,
  ) {
    db.get((error, connection) => {
      ...
      if (ipobj > 0) {
        ...
        sql =
          'select A.type from interface O ' +
  ...
  public static getPositionsContent = (
    dbCon: Query,
    position: number,
    new_position: number,
    ...
    return new Promise((resolve, reject) => {
      dbCon.query(
        ...
        (error, result: Array<{ id: number; content: string }>) => {
          if (error) return reject(error);
          if (result.length !== 1) reject(fwcError.NOT_FOUND);
          const content1 = result[0].content;

          dbCon.query(
            `SELECT id, content FROM policy_position WHERE id=${new_position}`,
            (error, result: Array<{ id: number; content: string }>) => {
              if (error) return reject(error);
              if (result.length !== 1) reject(fwcError.NOT_FOUND);
  ...
  public static deletePolicy_r__ipobj(
    ...
    return new Promise((resolve, reject) => {
      const sqlExists = `SELECT * FROM ${tableModel}
                WHERE rule=${dbCon.escape(rule)} AND ipobj=${dbCon.escape(ipobj)}
                AND ipobj_g=${dbCon.escape(ipobj_g)} AND position=${dbCon.escape(position)}`;
      dbCon.query(sqlExists, (error, row: Array<PolicyRuleToIPObj>) => {
        if (error) return reject(error);
  ...
  public static deletePolicy_r__All(
    rule: number,
    callback: (err: Error | null, result: { result: boolean; msg?: string } | null) => void,
  ) {
    db.get((error, connection) => {
      if (error) callback(error, null);
      const sqlExists = 'SELECT * FROM ' + tableModel + ' WHERE rule = ' + connection.escape(rule);
      connection.query(sqlExists, (error, row: Array<PolicyRuleToIPObj>) => {
        //If exists Id from policy_r__ipobj to remove
        if (row) {
          logger().debug('DELETING IPOBJ FROM RULE: ' + rule);
          db.get((error, connection) => {
            const sql = 'DELETE FROM ' + tableModel + ' WHERE rule = ' + connection.escape(rule);
  ...
  public static orderPolicyPosition(
    rule: number,
    position: number,
    callback: (err: Error | null, result: { result: boolean } | null) => void,
  ) {
    ...
    db.get((error, connection) => {
      ...
      const sqlPos =
        ...
        ' order by position_order';
      logger().debug(sqlPos);
      connection.query(sqlPos, (error, rows: Array<PolicyRuleToIPObj>) => {
        if (rows.length > 0) {
          let order = 0;
          asyncMod.map(
            rows,
            (row, callback1: Function) => {
              ...
              db.get((error, connection) => {
                const sql =
                  ...
                  ' AND ipobj=' +
                  connection.escape(row.ipObjId) +
                  ' AND ipobj_g=' +
                  connection.escape(row.ipObjGroupId) +
                  ' AND position=' +
  ...
  public static orderPolicy(
    rule: number,
    callback: (err: Error | null, result: { result: boolean }) => void,
  ) {
    db.get((error, connection) => {
      if (error) callback(error, null);
      const sqlRule =
      ...
      connection.query(sqlRule, (error, rows: Array<PolicyRuleToIPObj>) => {
        if (rows.length > 0) {
          let order = 0;
          let prev_position = 0;
          asyncMod.map(
  ...
  //Order policy_r__ipobj Position
  public static orderAllPolicy(callback: (err: Error | null, result: { result: boolean }) => void) {
    db.get((error, connection) => {
      if (error) callback(error, null);
  ...
  public static checkIpobjInRule(
    ipobj: number,
    type: number,
    fwcloud: number,
    callback: (err: Error | null, result: { result: boolean }) => void,
      ...
      connection.query(sql, (error, rows: Array<{ n: number }>) => {
        if (!error) {
          if (rows.length > 0) {
            if (rows[0].n > 0) {
              logger().debug(
                'ALERT <INTERFACE> DELETING interface IN RULE:' +
                  _interface +
                  ' type: ' +
                  type +
                  ' fwcloud:' +
      ...
      connection.query(sql, (error, rows: Array<{ n: number }>) => {
        if (!error) {
          if (rows.length > 0) {
            if (rows[0].n > 0) {
              logger().debug(
                'ALERT <HOST ALL INTERFACES> DELETING interface IN RULE:' +
                  ipobj_host +
                  ' fwcloud:' +
                  fwcloud +
                  ' --> FOUND IN ' +
  ...
  public static checkHostAllInterfaceAllIpobjInRule(
    ipobj_host: number,
    fwcloud: number,
    callback: (err: Error | null, result: { result: boolean }) => void,
  ) {
    ...
    db.get((error, connection) => {
      if (error) callback(error, null);
      const sql =
        'SELECT count(*) as n FROM interface__ipobj J ' +
      ...
      connection.query(sql, (error, rows: Array<{ n: number }>) => {
        if (!error) {
          if (rows.length > 0) {
            if (rows[0].n > 0) {
              logger().debug(
                'ALERT <HOST ALL IPOBJ ALL INTERFACES> DELETING interface IN RULE:' +
                  ipobj_host +
                  ' fwcloud:' +
                  fwcloud +
                  ' --> FOUND IN ' +
      ...
      connection.query(sql, (error, rows: Array<{ n: number }>) => {
        if (!error) {
          if (rows.length > 0) {
            if (rows[0].n > 0) {
              logger().debug(
                'ALERT <IPOBJ UNDER INTERFACE> DELETING interface IN RULE:' +
                  _interface +
                  ' type: ' +
                  type +
                  ' fwcloud:' +
      ...
      connection.query(sql, (error, rows: Array<{ n: number }>) => {
        if (!error) {
          if (rows.length > 0) {
            if (rows[0].n > 0) {
              logger().debug(
                'ALERT <HOST INTERFACE> DELETING interface IN RULE:' +
                  _interface +
                  ' type: ' +
                  type +
                  ' fwcloud:' +
  ...
  //check if IPOBJ Exists in any rule
  public static searchIpobjInRule = (
    ipobj: number,
    type: number,
    fwcloud: number,
  ...
  public static searchInterfaceInRule = (
    _interface: number,
    type: string,
    fwcloud: number,
    firewall: number,
  ...
  public static searchIpobjInGroupInRule = (
    ipobj: number,
    type: number,
    fwcloud: number,
  ): Promise<Array<PolicyRuleToIPObjInRuleData>> => {
  ...
  public static searchGroupIPObjectsInRule = (
    idg: number,
    fwcloud: number,
  ): Promise<Array<PolicyRuleToIPObjInRuleData>> => {
    return new Promise((resolve, reject) => {
  ...
  public static searchGroupInRule = (
    idg: number,
    fwcloud: number,
  ): Promise<Array<PolicyRuleToIPObjInRuleData>> => {
    return new Promise((resolve, reject) => {
  ...
  public static searchInterfaceHostInRule = (
    dbCon: Query,
    fwcloud: number,
    ipobj: number,
  ): Promise<Array<PolicyRuleToIPObjInRuleData>> => {
    return new Promise((resolve, reject) => {
      ...
      dbCon.query(sql, (error: Error, rows: Array<PolicyRuleToIPObjInRuleData>) => {
        if (error) return reject(error);
        resolve(rows);
      });
    });
  ...
  public static searchAddrHostInRule = (
    dbCon: Query,
    fwcloud: number,
    ipobj: number,
  ): Promise<Array<PolicyRuleToIPObjInRuleData>> => {
      ...
      dbCon.query(sql, async (error: Error, rows: Array<PolicyRuleToIPObjInRuleData>) => {
        ...
        try {
          for (const row of rows) {
            ...
            const addrs = await Interface.getInterfaceAddr(dbCon, row.obj_id);

            // Count the amount of interface address with the same IP version of the rule.
            let n = 0;
            let id = 0;
            for (const addr of addrs) {
              if (addr.ip_version === rule_ip_version) {
                n++;
                if (n === 1) id = addr.id;
              }
            }
            ...
            if (n === 1 && ipobj === id) result.push(row);
          }
        } catch (error) {
          return reject(error);
        }
  ...
  public static searchLastAddrInHostInRule = (
    ...
    return new Promise((resolve, reject) => {
      ...
      const sql = `SELECT O.ipobj obj_id,IR.name obj_name, IR.type obj_type_id,T.type obj_type_name,
			...
			INNER JOIN fwcloud C ON C.id=F.fwcloud
			WHERE I.id=${ipobj} AND I.type=${type} AND F.fwcloud=${fwcloud}`;
      dbCon.query(sql, async (error, rows: Array<PolicyRuleToIPObjInRuleData>) => {
        if (error) return reject(error);

        const result: Array<PolicyRuleToIPObjInRuleData> = [];
        try {
          for (const row of rows) {
            const rule_ip_version = await PolicyRule.getPolicyRuleIPversion(
              dbCon,
            ...
            const addrs = await Interface.getHostAddr(dbCon, row.obj_id);
  ...
  public static async searchLastAddrInHostInGroup(
    ipObjId: number,
    type: number,
    fwcloudId: number,
  ): Promise<PolicyRule[]> {
    const policyRuleToIPObjGroups: PolicyRuleToIPObj[] = await db
      .getSource()
      .manager.getRepository(PolicyRuleToIPObj)
      .createQueryBuilder('policyRuleToIPObj')
    ...
    for (const policyRuleToIPObjGroup of policyRuleToIPObjGroups) {
      for (const ipObjToIPObjGroup of policyRuleToIPObjGroup.ipObjGroup.ipObjToIPObjGroups) {
        ...
        // We are the last IP address in the host used in a firewall rule.
        if (n === 1 && ipObjId === id) result.push(policyRuleToIPObjGroup);
      }
    }
    ...
    return await db
      ...
      .where('rule.id IN (:ids)', {
        ids: result.map((item) => item.policyRuleId),
      })
  ...
  public static searchIpobjInterfaceInRule = (
    ...
    return new Promise((resolve, reject) => {
      db.get((error, connection) => {
        if (error) return reject(error);
        let sql =
          'SELECT I.id obj_id,I.name obj_name, I.type obj_type_id,T.type obj_type_name, ' +
          ...
          ' WHERE K.id=' +
          _interface +
          ' AND K.interface_type=' +
          type +
          ' AND F.fwcloud=' +
          fwcloud;
        if (diff_firewall !== '') sql = sql + ' AND F.id<>' + connection.escape(diff_firewall);
        else if (firewall !== null) sql = sql + ' AND F.id=' + connection.escape(firewall);
        connection.query(sql, (error, rows: Array<PolicyRuleToIPObjInRuleData>) => {
          if (error) return reject(error);
          resolve(rows);
        });
  ...
  public static searchIpobjInterfaceInGroup = (
    _interface: number,
    type: string,
  ): Promise<Array<PolicyRuleToIPObjInRuleData>> => {
    return new Promise((resolve, reject) => {
