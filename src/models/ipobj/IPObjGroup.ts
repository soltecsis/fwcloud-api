...
export class IPObjGroup extends Model {
  ...
  public static groupIPVersion(
    ...
    return new Promise((resolve, reject) => {
      dbCon.query(
        `select type from ${tableName} where id=${group}`,
        async (error, result: Array<{ type: number }>) => {
          try {
            if (error) return reject(error);
  ...
  public static getIpobj_g_Full(
    dbCon: Query,
    fwcloud: number,
    gid: number,
  ): Promise<ipobj_g_Data[]> {
    return new Promise((resolve, reject) => {
      ...
      dbCon.query(
        sql,
        (error, rows: Array<IPObjGroup & { id_node: number; id_parent_node: number }>) => {
          if (error) return reject(error);
          if (rows.length === 0) return reject(fwcError.NOT_FOUND);

          const groups: ipobj_g_Data[] = [];
          ...
          dbCon.query(
            sql,
            async (error, rows: Array<{ id: number; name: string; type: string }>) => {
              if (error) return reject(error);
  ...
  //Get ipobj_g by  id AND ALL IPOBjs
  public static getIpobj_g_Full_Pro(fwcloud: number, id: string): Promise<ipobj_g_Data[]> {
    return new Promise((resolve, reject) => {
      const groups: ipobj_g_Data[] = [];
      let group_cont = 0;
      let ipobjs_cont = 0;
  ...
  //Add new ipobj_g
  public static insertIpobj_g(
    ipobj_gData: IPObjGroup,
