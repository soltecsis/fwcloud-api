...
export class Tree extends Model {
  ...
  public static getNodeInfo(
    ...
    return new Promise((resolve, reject) => {
      ...
      dbCon.query(sql, (error, result: Array<Tree>) => {
        if (error) return reject(error);
        resolve(result);
      });
    });
  ...
  public static getNodeUnderFirewall(
    dbCon: Query,
    fwcloud: number,
    firewall: number,
    node_type: string,
  ...
  public static dumpTree(dbCon: Query, treeType: TreeType, fwcloud: number): Promise<TreeNode> {
    return new Promise((resolve, reject) => {
      ...
      dbCon.query(sql, async (error, nodes: Array<TreeNode>) => {
        if (error) return reject(error);
  ...
  private static async addSearchInfoOpenVPN(node: TreeNode): Promise<TreeNode> {
  ...
  public static stdFoldersFirst(root_node: TreeNode): Promise<void> {
  ...
  public static deleteObjFromTree(
    fwcloud: number,
    id_obj: number,
    obj_type: number,
  ): Promise<void> {
  ...
  //DELETE NODE
  public static deleteFwc_Tree_node(id: number): Promise<{ result: boolean; msg: string }> {
    return new Promise((resolve, reject) => {
      db.get((error, connection) => {
        if (error) return reject(error);
  ...
  public static deleteNodesUnderMe(dbCon: Query, fwcloud: number, node_id: number): Promise<void> {
    return new Promise((resolve, reject) => {
  ...
  //Verify node info.
  public static verifyNodeInfo(id: number, fwcloud: number, id_obj: number): Promise<boolean> {
    return new Promise((resolve, reject) => {
      db.get((error, connection) => {
        if (error) return reject(error);
        const sql =
  ...
  //UPDATE NODE
  public static updateIDOBJFwc_Tree_node(
    fwcloud: number,
    id: number,
    idNew: number,
  ...
  public static async createObjectsTree(dbCon: Query, fwCloudId: number): Promise<objectTree> {
  ...
  public static createStdObjectsTree(
    dbCon: Query,
    node_id: number,
    node_type: string,
    ipobj_type: number,
  ...
  public static createGroupNodes(
    dbCon: Query,
    fwcloud: number,
    node_id: number,
    group: number,
    ...
    return new Promise((resolve, reject) => {
      let sql = `SELECT O.id id,O.name name,O.type type FROM ipobj__ipobjg G
			INNER JOIN ipobj O ON O.id=G.ipobj
			WHERE G.ipobj_g=${group}`;
      dbCon.query(sql, async (error, ipobjs: Array<{ id: number; name: string; type: number }>) => {
        if (error) return reject(error);
        ...
        sql = `SELECT VPN.id as id,CRT.cn as cn FROM openvpn__ipobj_g G
				INNER JOIN openvpn VPN ON VPN.id=G.openvpn
				INNER JOIN crt CRT ON CRT.id=VPN.crt
				WHERE G.ipobj_g=${group}`;
        dbCon.query(sql, async (error, openvpns: Array<{ id: number; cn: string }>) => {
          if (error) return reject(error);
          try {
          ...
          sql = `SELECT P.id id,P.name name FROM openvpn_prefix__ipobj_g G
					INNER JOIN openvpn_prefix P ON P.id=G.prefix
					WHERE G.ipobj_g=${group}`;
          dbCon.query(sql, async (error, prefixes: Array<{ id: number; name: string }>) => {
            if (error) return reject(error);
  ...
  public static createStdGroupsTree(
    dbCon: Query,
    node_id: number,
    node_type: string,
    ipobj_type: number,
    ...
    return new Promise((resolve, reject) => {
      ...
      dbCon.query(sql, async (error, groups: Array<{ id: number; name: string }>) => {
        if (error) return reject(error);
  ...
  public static interfacesIpTree(
    connection: Query,
    fwcloud: number,
    nodeId: number,
    ifId: number,
    ...
    return new Promise((resolve, reject) => {
      // Get interface IPs.
      const sql =
        'SELECT O.id id,O.name name,O.type type,O.address address ,T.node_type node_type FROM ipobj O' +
  ...
  public static interfacesTree(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (ownerType === 'FW') {
        ...
        sql =
          'SELECT I.id id,I.name name ,I.labelName labelName FROM interface I' +
      ...
      connection.query(
        sql,
        async (error, interfaces: Array<{ id: number; name: string; labelName: string }>) => {
          if (error) return reject(error);
  ...
  public static openvpnClientTree(
    connection: Query,
    fwcloud: number,
    firewall: number,
    server_vpn: number,
    ...
    return new Promise((resolve, reject) => {
      ...
      connection.query(sql, async (error, vpns: Array<{ id: number; cn: string }>) => {
        if (error) return reject(error);
  ...
  public static openvpnServerTree(
    connection: Query,
    fwcloud: number,
    firewall: number,
    node: number,
  ...
  public static async routingTree(
    ...
    const id3 = await this.newNode(connection, fwcloud, 'TABLES', id2, 'RTS', firewall, null);

    const sql = `SELECT id,name FROM routing_table WHERE firewall=${firewall}`;
    connection.query(sql, async (error, tables: Array<{ id: number; name: string }>) => {
  ...
  public static insertFwc_Tree_New_firewall(
    ...
    return new Promise((resolve, reject) => {
      db.get((error, connection) => {
        ...
        connection.query(sql, async (error, firewalls: Array<{ name: string }>) => {
          if (error) return reject(error);
    ...
    return new Promise((resolve, reject) => {
      db.get((error, connection) => {
        ...
        const sql =
          ...
          connection.escape(fwcloud) +
          ' AND node_type="FCF"';
        connection.query(sql, async (error, nodes: Array<{ id: number }>) => {
          if (error) return reject(error);
    ...
    return new Promise((resolve, reject) => {
      db.get((error, connection) => {
        ...
        connection.query(
          ...
          async (
            error: Error,
            clusters: Array<{ id: number; name: string; fwmaster_id: number }>,
          ) => {
            if (error) return reject(error);
            ...
            try {
              ...
              sql =
                'SELECT id,name FROM firewall WHERE cluster=' +
                clusterId +
                ' AND fwcloud=' +
                fwcloud;
              connection.query(
                sql,
                async (error, firewalls: Array<{ id: number; name: string }>) => {
                  if (error) return reject(error);
  ...
  public static updateFwc_Tree_convert_firewall_cluster(
    ...
    db.get((error, connection) => {
      ...
      this.getFirewallNodeId(idfirewall, (error, datafw) => {
        ...
        logger().debug(sql);
        connection.query(sql, (error, rows: Array<Tree>) => {
        ...
        connection.query(sql, (error, rows: Array<Tree>) => {
          ...
          if (rows && rows.length > 0) {
            ...
            const sql =
              ...
              ' AND id_obj=' +
              idcluster;
            connection.query(sql, (error, rowsCL: Array<Tree>) => {
              if (error) {
                AllDone(error, null);
                ...
                const sql =
                  ...
                  connection.escape(fwcloud);
                logger().debug(sql);
                connection.query(sql, (error, rowsN: Array<Tree>) => {
                  if (error) {
                    AllDone(error, null);
  ...
  public static updateFwc_Tree(
    nodeTreeData: Tree,
    callback: (error: Error, result: { result: boolean }) => void,
  ) {
    db.get((error, connection) => {
      if (error) callback(error, null);
      const sql =
  ...
  public static updateFwc_Tree_Firewall(
    dbCon: Query,
    fwcloud: number,
    FwData: firewalls_Data,
  ): Promise<void> {
    return new Promise((resolve, reject) => {
  ...
  public static updateFwc_Tree_OBJ(
    ...
    return new Promise((resolve, reject) => {
      ...
      // Firewall and host interfaces.
      if ((ipobjData.type === 10 || ipobjData.type === 11) && ipobjData.label_name)
        name += ' [' + ipobjData.label_name + ']';
      // Interface address.
      ...
      req.dbCon.query(sql, (error, result: { affectedRows: number }) => {
        if (error) return reject(error);
  ...
  public static deleteFwc_TreeGroupChild(
    dbCon: Query,
    fwcloud: number,
    id_group: number,
    id_obj: number,
  ...
  public static orderTreeNodeDeleted(dbCon: Query, fwcloud: number, id_obj_deleted: number) {
    return new Promise((resolve, reject) => {
      const sqlParent =
        ...
        if (rows.length > 0) {
          asyncMod.map(
            ...
            (row, callback1: () => void) => {
              ...
              dbCon.query(sqlNodes, (error, rowsnodes: Array<Tree>) => {
                if (error) return reject(error);
  ...
  public static orderTreeNode(
    ...
    db.get((error, connection) => {
      ...
      const sqlNodes =
        ...
        '  order by node_order';
      logger().debug(sqlNodes);
      connection.query(sqlNodes, (error, rowsnodes: Array<Tree>) => {
        if (rowsnodes.length > 0) {
          let order = 0;
          asyncMod.map(
            rowsnodes,
