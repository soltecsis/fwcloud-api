...
export class Repair extends Model {
            ...
            dbCon.query(sql, async (error, nodes) => {
                ...
                for (const node of nodes) {
                    if (node.name === 'FIREWALLS' && node.node_type === 'FDF') {
                        ...
                        channel.emit('message', new ProgressNoticePayload(`Deleting invalid root node: ${node.id}\n`));
                ...
                if (update_obj_to_null) {
                    channel.emit('message', new ProgressNoticePayload(`Repairing root nodes (setting id_obj and obj_type to null).\n`));
    ...
    public static checkNotRootNodes(rootNodes, channel: EventEmitter = new EventEmitter()): Promise<void> {
        return new Promise((resolve, reject) => {
            const sql = 'SELECT id,id_parent,name,node_type,id_obj,obj_type FROM ' + tableName +
                    ...
                    for (const node of nodes) {
                        ...
                        do {
                            ...
                            if (id_ancestor === -1 || id_ancestor === node.id || (++deep) > 100) {
                                ...
                                if (id_ancestor === -1) {
                                    channel.emit('message', new ProgressNoticePayload(`Ancestor not found, deleting node: ${node.id}\n`));
                                } else if (id_ancestor === node.id) {
                                    channel.emit('message', new ProgressNoticePayload(`Deleting node in a loop: ${node.id}\n`));
                                } else if (deep > 100) {
                                    channel.emit('message', new ProgressNoticePayload(`Deleting a too much deep node: ${node.id}\n`));
                                }
                        ...
                        if (!root_node_found) {
                            channel.emit('message', new ProgressNoticePayload(`Root node for this node is not correct. Deleting node: ${node.id}\n'`));
                            await Tree.deleteFwc_TreeFullNode({ id: node.id, fwcloud: fwcloud });
                            continue;
                        }
                    }
    ...
    public static regenerateFirewallTree(rootNode, firewall, channel: EventEmitter = new EventEmitter()): Promise<void> {
        return new Promise((resolve, reject) => {
            const sql = 'SELECT T1.id,T1.id_parent,T2.node_type as parent_node_type FROM fwc_tree T1' +
            ...
            dbCon.query(sql, async (error, nodes) => {
                ...
                try {
                    ...
                    if (nodes.length === 0) {
                        ...
                        channel.emit('message', new ProgressNoticePayload(`No node found for firewall: ${JSON.stringify(firewall)}\n`));
                    } else {
                        if (nodes.length === 1) { // The common case, firewall referenced by only one node three.
                            ...
                            channel.emit('message', new ProgressNoticePayload(`Found several nodes for firewall: ${JSON.stringify(firewall)}\n`));
                        }
                    ...
                    // Regenerate the tree.
                    channel.emit('message', new ProgressNoticePayload(`Regenerating tree for firewall: ${firewall.id} \n`));
    ...
    public static checkFirewallsInTree(rootNode, channel: EventEmitter = new EventEmitter()): Promise<void> {
        return new Promise((resolve, reject) => {
            const sql = 'SELECT id,name,options FROM firewall WHERE cluster is null AND fwcloud=' + dbCon.escape(fwcloud);
            ...
            dbCon.query(sql, async (error, nodes) => {
                ...
                try {
                    ...
                    if (nodes.length === 0) {
                        ...
                        channel.emit('message', new ProgressNoticePayload(`No node found for cluster: ${JSON.stringify(cluster)}\n`));
                    } else {
                        if (nodes.length === 1) { 
                            ...
                            channel.emit('message', new ProgressNoticePayload(`Found several nodes for cluster: ${JSON.stringify(cluster)}\n`));
                        }
                    ...
                    // Regenerate the tree.
                    channel.emit('message', new ProgressNoticePayload(`Regenerating tree for cluster: ${cluster.id} \n`));
    ...
    public static checkClustersInTree(rootNode, channel: EventEmitter = new EventEmitter()): Promise<void> {
        return new Promise((resolve, reject) => {
            const sql = 'SELECT C.id,C.name,F.id as fwmaster_id,F.options FROM cluster C ' +
            ...
            try {
                ...
                if (node.node_type === 'FW') {
                    if (node.obj_type !== 0) { // Verify that object type is correct.
                        channel.emit('message', new ProgressNoticePayload(`Deleting node with bad obj_type: ${JSON.stringify(node)}\n`));
                        await Tree.deleteFwc_TreeFullNode({ id: node.id, fwcloud: fwcloud });
                ...
                dbCon.query(sql, async (error, rows) => {
                    ...
                    if (rows.length !== 1) {
                        channel.emit('message', new ProgressNoticePayload(`Referenced object not found. Deleting node: ${JSON.stringify(node)}\n`));
                        await Tree.deleteFwc_TreeFullNode({ id: node.id, fwcloud: fwcloud });
                        resolve(false);
                    } else resolve(true);
                });
    ...
    public static checkFirewallsFoldersContent(rootNode, channel: EventEmitter = new EventEmitter()): Promise<void> {
        return new Promise((resolve, reject) => {
            const sql = 'SELECT id,node_type,id_obj,obj_type FROM ' + tableName +
                ...
                try {
                    for (const node of nodes) {
                        ...
                        if (node.node_type !== 'FD' && node.node_type !== 'FW' && node.node_type !== 'CL') {
                            channel.emit('message', new ProgressNoticePayload(`This node type can not be into a folder. Deleting it: ${JSON.stringify(node)}\n`));
                            await Tree.deleteFwc_TreeFullNode({ id: node.id, fwcloud: fwcloud });
                        }
                        ...
                        if (node.node_type === 'FW' || node.node_type === 'CL')
                            ...
                            channel.emit('message', new ProgressNoticePayload(`Checking folder node: ${JSON.stringify(node)} \n`));
