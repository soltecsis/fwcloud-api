...
import { EndTaskPayload, ErrorTaskPayload, StartTaskPayload } from "../../fonaments/http/progress/messages/progress-messages";
...
export class BackupRestoreCommand extends Command {
    ...
    async handle(args: yargs.Arguments) {
        const backupService: BackupService = await this._app.getService<BackupService>(BackupService.name);
    ...
    public getArguments(): Argument[] {
        return [
            { name: 'id', description: 'Backup id', required: true },
        ]
    }
}