...
export class changeUniqueKeyOpenvpn1653307616404 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        const indexUniqueKey = table.indices.findIndex(item => item.columnNames = ['firewall', 'crt'])
        ...
        await queryRunner.createIndex('openvpn', new TableIndex({
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        const indexUniqueKey = table.indices.findIndex(item => item.columnNames = ['openvpn', 'crt'])
        await queryRunner.dropIndex('openvpn', table.indices[indexUniqueKey])
