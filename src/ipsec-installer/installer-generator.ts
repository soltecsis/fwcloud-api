...
export class InstallerGenerator {
  ...
  constructor(workspace: string, name: string, configData: string, outputPath: string) {
    ...
    this._osslsigncodePath =
      app().config.get('ipsec.installer.osslsigncode.path') ?? this.guessOsslSignCodePath();
  }
  ...
  public generate(sign: boolean = true): string {
    try {
      ...
      if (this.shouldSignExecutable() && sign) {
        const certPath: string = app().config.get('ipsec.installer.osslsigncode.cert_path');
        const url: string = app().config.get('ipsec.installer.osslsigncode.url');
        const description: string = app().config.get('ipsec.installer.osslsigncode.description');

        try {
          this.signExecutable(this._osslsigncodePath, certPath, url, description);
        } catch (e) {
          logger().error('Error during ipsec installer signing: ' + e.message);
          logger().info('Creating executable unsigned');
          return this.generate(false);
        }
      } else {
        logger().info('IPSec installer executable unsigned generated');
      }
  ...
  public shouldSignExecutable(): boolean {
    ...
    if (!this.osslsignCertificateExists()) {
      logger().warn(
        'IPSec installer sign certificate does not exists. Path was: ' +
          app().config.get('ipsec.installer.osslsigncode.cert_path'),
      );

      return false;
    }

    if (app().config.get('ipsec.installer.osslsigncode.url') !== null) {
      logger().warn(`osslsigncode "i" argument missing`);

      return false;
    }

    if (app().config.get('ipsec.installer.osslsigncode.description') !== null) {
