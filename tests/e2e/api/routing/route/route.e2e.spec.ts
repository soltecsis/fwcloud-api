      ...
      it('regular user which belongs to the fwcloud should move routes', async () => {
        ...
        expect(
          (
            await manager
              .getRepository(Route)
              .findOne({ where: { id: routeOrder1.id } })
          ).route_order,
        ).to.eq(1);
        expect(
          (
            await manager
              .getRepository(Route)
              .findOne({ where: { id: routeOrder2.id } })
          ).route_order,
        ).to.eq(2);
        expect(
          (
            await manager
              .getRepository(Route)
              .findOne({ where: { id: routeOrder3.id } })
          ).route_order,
        ).to.eq(3);
        expect(
          (
            await manager
              .getRepository(Route)
              .findOne({ where: { id: routeOrder4.id } })
          ).route_order,
        ).to.eq(4);
      });
      ...
      it('regular user which belongs to the fwcloud should move from items to gateway between rules', async () => {
        ...
        expect(
          (
            await manager
              .getRepository(Route)
              .findOneOrFail({ where: { id: route2.id } })
          ).gatewayId,
        ).to.eq(gateway.id);
      });
    ...
    describe('@copy', () => {
      ...
      beforeEach(async () => {
        ...
        data = {
          routes: [routeOrder1.id, routeOrder2.id],
          to: (
            await new RouteRepository(manager).getLastRouteInRoutingTable(
              table.id,
            )
          ).id,
          offset: Offset.Below,
