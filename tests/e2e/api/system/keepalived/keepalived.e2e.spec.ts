  ...
  describe(KeepalivedController.name, () => {
    ...
    describe("@move", () => {
      ...
      it("regular user which belongs to the fwcloud should move a Keepalived rule", async () => {
        ...
        expect(
          (
            await getRepository(KeepalivedRule).findOneOrFail(
              KeepalivedRule1.id,
            )
          ).rule_order,
        ).to.equal(1);
    ...
    describe("@bulkUpdate", () => {
      ...
      it("regular user which belongs to the fwcloud should bulk update a Keepalived rule", async () => {
        ...
        expect(
          (await getRepository(KeepalivedRule).findOneOrFail(rule1.id)).active,
        ).to.equal(false);
        expect(
          (await getRepository(KeepalivedRule).findOneOrFail(rule2.id)).active,
        ).to.equal(false);
      });
    ...
    describe("@bulkRemove", () => {
      ...
      it("regular user which belongs to the fwcloud should bulk remove a Keepalived rule", async () => {
        ...
        expect(await getRepository(KeepalivedRule).findOne(rule1.id)).to.be
          .undefined;
        expect(await getRepository(KeepalivedRule).findOne(rule2.id)).to.be
          .undefined;
      });
