...
describe(describeName('Ipobj group delfrom E2E Tests'), () => {
  ...
  beforeEach(async () => {
    ...
    routingRule = await routingRuleService.create({
      ...
      markIds: [{ id: fwcProduct.mark.id, order: 1 }],
    });
    ...
    describe('used in policy rules', () => {
      ...
      describe('INPUT', () => {
        beforeEach(async () => {
          rule = await manager
            ...
            .findOneOrFail({ where: { id: inputRuleId } });
        });
      ...
      describe('OUTPUT', () => {
        beforeEach(async () => {
          rule = await manager
            .getRepository(PolicyRule)
            .findOneOrFail({ where: { id: outputRuleId } });
        });
      ...
      describe('DNAT', () => {
        beforeEach(async () => {
          rule = await manager
            .getRepository(PolicyRule)
            .findOneOrFail({ where: { id: dnatRuleId } });
        });
      ...
      describe('SNAT', () => {
        beforeEach(async () => {
          rule = await manager
            .getRepository(PolicyRule)
            .findOneOrFail({ where: { id: snatRuleId } });
        });
