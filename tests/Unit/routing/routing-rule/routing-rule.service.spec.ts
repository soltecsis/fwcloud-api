...
describe(RoutingRuleService.name, () => {
  ...
  beforeEach(async () => {
    ...
    ctr = await manager.getRepository(Cluster).save(
  ...
  describe('create', () => {
    ...
    describe('IpObjs', () => {
      ...
      beforeEach(async () => {
        ipobj1 = await manager.getRepository(IPObj).save(
        ...
        ipobj2 = await manager.getRepository(IPObj).save(
          manager.getRepository(IPObj).create({
            name: 'test',
            address: '0.0.0.0',
            ipObjTypeId: 0,
    ...
    describe('IpObjGroups', () => {
      ...
      beforeEach(async () => {
        group1 = await manager.getRepository(IPObjGroup).save(
        ...
        group2 = await manager.getRepository(IPObjGroup).save(
          manager.getRepository(IPObjGroup).create({
            name: StringHelper.randomize(10),
            type: 20,
            fwCloudId: fwCloud.id,
    ...
    describe('OpenVPNs', () => {
      ...
      beforeEach(async () => {
        openVPN1 = await manager.getRepository(OpenVPN).save(
        ...
        openVPN2 = await manager.getRepository(OpenVPN).save(
          manager.getRepository(OpenVPN).create({
            firewallId: firewall.id,
            parentId: fwcProduct.openvpnServer.id,
            crt: await manager.getRepository(Crt).save(
    ...
    describe('OpenVPNPrefixes', () => {
      ...
      beforeEach(async () => {
        openVPNPrefix = await manager.getRepository(OpenVPNPrefix).save(
          manager.getRepository(OpenVPNPrefix).create({
            name: StringHelper.randomize(10),
            openVPN: await manager.getRepository(OpenVPN).save(
              manager.getRepository(OpenVPN).create({
                firewallId: firewall.id,
                crt: await manager.getRepository(Crt).save(
                  manager.getRepository(Crt).create({
        ...
        openVPNPrefix2 = await manager.getRepository(OpenVPNPrefix).save(
          manager.getRepository(OpenVPNPrefix).create({
            name: StringHelper.randomize(10),
            openVPN: await manager.getRepository(OpenVPN).save(
              manager.getRepository(OpenVPN).create({
  ...
  describe('moveFrom', () => {
    ...
    describe('ipObj', () => {
      it('should move ipObj', async () => {
        ...
        const refreshedRule1: RoutingRule = await manager
          .getRepository(RoutingRule)
          .findOne({
            where: {
              id: rule1.id,
        ...
        const refreshedRule2: RoutingRule = await manager
          .getRepository(RoutingRule)
          .findOne({
            where: {
              id: rule2.id,
