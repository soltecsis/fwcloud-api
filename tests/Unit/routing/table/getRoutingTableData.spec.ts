  ...
  describe('For grid', () => {
    ...
    describe('Out of group', () => {
      ...
      it('should include address data', async () => {
        ...
        item.host_name = null;
        item._order = (
          await manager.getRepository(RouteToIPObj).findOneOrFail({
            where: {
      ...
      it('should include OpenVPN data', async () => {
        ...
        item._order = (
          await manager.getRepository(RouteToOpenVPN).findOneOrFail({
            where: {
              routeId: fwc.routes.get('route1').id,
              openVPNId: item.id,
            },
      ...
      it('should include OpenVPN Prefix data', async () => {
        ...
        item.firewall_name = fwc.firewall.name;
        item._order = (
          await manager.getRepository(RouteToOpenVPNPrefix).findOneOrFail({
            where: {
              routeId: fwc.routes.get('route1').id,
    ...
    describe('Into group', () => {
      ...
      it('should include group', async () => {
        ...
        item._order = (
          await manager.getRepository(RouteToIPObjGroup).findOneOrFail({
            where: {
