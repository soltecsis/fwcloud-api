...
import { PolicyRuleToIPSec } from '../../../../src/models/policy/PolicyRuleToIPSec';
...
import { PolicyRuleToIPSecPrefix } from '../../../../src/models/policy/PolicyRuleToIPSecPrefix';
import {
...
describe(describeName('Policy Compiler Unit Tests - IPSec'), () => {
  let dbCon: any;
  let fwcloud: number;
  let vpnSrv: number;
  let vpnCli1: number;
  ...
  async function runTest(policyType: number, rulePosition: number, cs: string): Promise<void> {
    ...
    if (!useGroup) {
      if (!usePrefix) {
        await manager.getRepository(PolicyRuleToIPSec).save(
          manager.getRepository(PolicyRuleToIPSec).create({
            policyRuleId: rule,
            ipsecId: vpnCli1,
        ...
        await manager.getRepository(PolicyRuleToIPSecPrefix).save(
          manager.getRepository(PolicyRuleToIPSecPrefix).create({
            policyRuleId: rule,
            ipsecPrefixId: vpnPrefix,
            policyPositionId: rulePosition,
            position_order: 1,
          }),
        );
  ...
  before(async () => {
    ...
    vpnSrv = (
      await manager
        .getRepository(IPSec)
        .save(manager.getRepository(IPSec).create({ firewallId: ruleData.firewall, crtId: crtSrv }))
    ).id;
    vpnPrefix = (
      await manager
        .getRepository(IPSecPrefix)
        .save(manager.getRepository(IPSecPrefix).create({ ipsecId: vpnSrv, name: 'SOLTECSIS-' }))
    ).id;
    vpnCli1 = (
    ...
    await manager.getRepository(IPSecOption).save(
  ...
  describe('IPSec in policy rule (IPTables)', () => {
    before(() => {
      IPv = 'IPv4';
      compiler = 'IPTables';
      useGroup = false;
    ...
    describe('in INPUT chain', () => {
      ...
      it('should include the IPSec client IP in compilation string (destination position)', async () => {
        await runTest(
          PolicyTypesMap.get(`${IPv}:${policy}`),
    ...
    describe('in SNAT', () => {
      ...
      it('should include the IPSec client IP in compilation string (translated source)', async () => {
        await runTest(
          PolicyTypesMap.get(`${IPv}:${policy}`),
    ...
    describe('in DNAT', () => {
      ...
      it('should include the IPSec client IP in compilation string (translated destination)', async () => {
        await runTest(
          PolicyTypesMap.get(`${IPv}:${policy}`),
  ...
  describe('IPSec in policy rule (NFTables)', () => {
    before(() => {
      IPv = 'IPv4';
      compiler = 'NFTables';
      useGroup = false;
  ...
  describe('IPSec in group in policy rule (IPTables)', () => {
    before(async () => {
      IPv = 'IPv4';
      compiler = 'IPTables';
      useGroup = true;
      ...
      await IPSec.addToGroup(dbCon, vpnCli1, group);
    });
  ...
  describe('IPSec in group in policy rule (NFTables)', () => {
    before(async () => {
      IPv = 'IPv4';
      compiler = 'NFTables';
      useGroup = true;
  ...
  describe('IPSec prefix in policy rule (IPTables)', () => {
    before(() => {
      IPv = 'IPv4';
      compiler = 'IPTables';
      useGroup = false;
  ...
  describe('IPSec prefix in policy rule (NFTables)', () => {
    before(() => {
      IPv = 'IPv4';
      compiler = 'NFTables';
      useGroup = false;
  ...
  describe('IPSec in group in policy rule (IPTables)', () => {
    before(async () => {
      ...
      await IPSecPrefix.addPrefixToGroup(dbCon, vpnPrefix, group);
    });
  ...
  describe('IPSec prefix with several IPs', () => {
    before(async () => {
      vpnCli2 = (
        await manager.getRepository(IPSec).save(
          manager.getRepository(IPSec).create({
