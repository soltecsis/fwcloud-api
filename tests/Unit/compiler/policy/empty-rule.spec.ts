...
describe(describeName('Policy Compiler Unit Tests - Empty rule'), () => {
    ...
    async function runTest(policyType: number): Promise<void> {
        ...
        if (policyType === PolicyTypesMap.get(`${IPv}:DNAT`)) { 
            ...
            if (compiler === 'IPTables') {
                action = (policyType===PolicyTypesMap.get(`${IPv}:SNAT`)) ? 'MASQUERADE' : CompilerAction.get(`${compiler}:${ruleData.action}`);
                if (action==='ACCOUNTING') action = 'RETURN';
                st = (ruleData.action===RuleActionsMap.get('ACCEPT') && ruleData.options&0x0001 && policyType!==PolicyTypesMap.get(`${IPv}:SNAT`) && policyType!==PolicyTypesMap.get(`${IPv}:DNAT`)) ? '-m conntrack --ctstate NEW ' : '' ;
                
                // Accounting ,logging and marking is not allowed with SNAT and DNAT chains.
                log = (ruleData.options&0x0004 && policyType!==PolicyTypesMap.get(`${IPv}:SNAT`) && policyType!==PolicyTypesMap.get(`${IPv}:DNAT`)) ? `${cmd} -N FWCRULE${rule}.LOG\n${cmd} -A FWCRULE${rule}.LOG -m limit --limit 60/minute -j LOG --log-level info --log-prefix \"RULE ID ${rule} [${action}] \"\n${cmd} -A FWCRULE${rule}.LOG -j ${action}\n`: '';
                ...
                action = (policyType===PolicyTypesMap.get(`${IPv}:SNAT`)) ? 'counter masquerade' : CompilerAction.get(`${compiler}:${ruleData.action}`);
                if (action==='ACCOUNTING') action = 'counter return';
                st = (ruleData.action===RuleActionsMap.get('ACCEPT') && ruleData.options&0x0001 && policyType!==PolicyTypesMap.get(`${IPv}:SNAT`) && policyType!==PolicyTypesMap.get(`${IPv}:DNAT`)) ? 'ct state new ' : '' ;
                
                // Accounting ,logging and marking is not allowed with SNAT and DNAT chains.
                log = (ruleData.options&0x0004 && policyType!==PolicyTypesMap.get(`${IPv}:SNAT`) && policyType!==PolicyTypesMap.get(`${IPv}:DNAT`)) ? `${cmd} add chain ${family} filter FWCRULE${rule}.LOG\n${cmd} add rule ${family} filter FWCRULE${rule}.LOG limit rate 1/second burst 5 packets counter log prefix \\\"RULE ID ${rule} [${action}]\\\" level info\n${cmd} add rule ${family} filter FWCRULE${rule}.LOG ${action}\n`: '';
    ...
    describe('Empty rule (IPv4, IPTables)', () => {
        beforeEach(() => { IPv = 'IPv4'; compiler = 'IPTables'; });
    ...
    describe('Empty rule (IPv4, NFTables)', () => {
        beforeEach(() => { IPv = 'IPv4'; compiler = 'NFTables'; });
    ...
    describe('Empty rule in (IPv6, IPTables)', () => {
        beforeEach(() => { IPv = 'IPv6'; compiler = 'IPTables'; });
    ...
    describe('Empty rule in (IPv6, NFTables)', () => {
        beforeEach(() => { IPv = 'IPv6'; compiler = 'NFTables'; });
