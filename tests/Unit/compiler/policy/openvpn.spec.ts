...
describe(describeName('Policy Compiler Unit Tests - OpenVPN'), () => {
  ...
  async function runTest(
    ...
    if (!useGroup) {
      if (!usePrefix) {
        await manager.getRepository(PolicyRuleToOpenVPN).save(
          manager.getRepository(PolicyRuleToOpenVPN).create({
            policyRuleId: rule,
            openVPNId: vpnCli1,
        ...
        await manager.getRepository(PolicyRuleToOpenVPNPrefix).save(
          manager.getRepository(PolicyRuleToOpenVPNPrefix).create({
            policyRuleId: rule,
