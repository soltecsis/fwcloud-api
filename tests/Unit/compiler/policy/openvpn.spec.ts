...
describe(describeName('Policy Compiler Unit Tests - OpenVPN'), () => {
  ...
  async function runTest(
    ...
    if (!useGroup) {
      if (!usePrefix) {
        await getRepository(PolicyRuleToOpenVPN).save(
          getRepository(PolicyRuleToOpenVPN).create({
            policyRuleId: rule,
            openVPNId: vpnCli1,
        ...
        await getRepository(PolicyRuleToOpenVPNPrefix).save(
          getRepository(PolicyRuleToOpenVPNPrefix).create({
            policyRuleId: rule,
      ...
      await getRepository(PolicyRuleToIPObj).save(
        getRepository(PolicyRuleToIPObj).create({
          policyRuleId: rule,
          ipObjId: -1,
          ipObjGroupId: group,
          interfaceId: -1,
  ...
  before(async () => {
    ...
    await getRepository(OpenVPNOption).save(
  ...
  describe('OpenVPN in group in policy rule (IPTables)', () => {
    before(async () => {
      ...
      usePrefix = false;
      group = (
        await getRepository(IPObjGroup).save(
