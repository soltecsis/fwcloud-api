    ...
    describe('searchIpobjUsage', () => {
        describe('route', () => {
            ...
            it('should detect address range usages', async () => {
                const whereUsed: any = await IPObj.searchIpobjUsage(db.getQuery(), fwcloudProduct.fwcloud.id, fwcloudProduct.ipobjs.get('addressRange').id, 5);
                expect(whereUsed.restrictions.IpobjInRoute.map(route => route.route_id)).to.contains(fwcloudProduct.routes.get('route1').id);
            });

            it('should detect network in CIDR notation usages', async () => {
                const whereUsed: any = await IPObj.searchIpobjUsage(db.getQuery(), fwcloudProduct.fwcloud.id, fwcloudProduct.ipobjs.get('network').id, 5);
                expect(whereUsed.restrictions.IpobjInRoute.map(route => route.route_id)).to.contains(fwcloudProduct.routes.get('route1').id);
            });

            it('should detect network not in CIDR notation usages', async () => {
                const whereUsed: any = await IPObj.searchIpobjUsage(db.getQuery(), fwcloudProduct.fwcloud.id, fwcloudProduct.ipobjs.get('networkNoCIDR').id, 5);
                expect(whereUsed.restrictions.IpobjInRoute.map(route => route.route_id)).to.contains(fwcloudProduct.routes.get('route1').id);
            });
 
            it('should detect host usages', async () => {
                const whereUsed: any = await IPObj.searchIpobjUsage(db.getQuery(), fwcloudProduct.fwcloud.id, fwcloudProduct.ipobjs.get('host').id, 5);
                expect(whereUsed.restrictions.IpobjInRoute.map(route => route.route_id)).to.contains(fwcloudProduct.routes.get('route1').id);
            });
