...
describe(DHCPGroupService.name, () => {
    ...
    describe('create', () => {
        ...
        it('should create a new DHCPGroup', async () => {
            ...
            const saveStub = sinon.stub(service['_repository'], 'save').resolves({ id: 1 } as DHCPGroup);
            const findOneStub2 = sinon.stub(service['_repository'], 'findOne').resolves({ id: 1 } as DHCPGroup);
        ...
        it('should handle errors during the creation process', async () => {
            ...
            const findOneStub = sinon.stub(getRepository(Firewall), 'findOne').resolves(firewall);
            const saveStub = sinon.stub(service['_repository'], 'save').rejects(new Error('Failed to create DHCPGroup'));
        ...
        it('should handle errors during the retrieval of the created group', async () => {
            ...
            const findOneStub = sinon.stub(getRepository(Firewall), 'findOne').resolves(firewall);
            const saveStub = sinon.stub(service['_repository'], 'save').resolves({ id: 1 } as DHCPGroup);
            const findOneStub2 = sinon.stub(service['_repository'], 'findOne').rejects(new Error('Failed to retrieve DHCPGroup'));
