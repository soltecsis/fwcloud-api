...
describe(DHCPRuleService.name, () => {
  ...
  describe('store', () => {
    ...
    it('should throw an error if the group does not exist', async () => {
      ...
      const findOneOrFailStub = sinon
        .stub(manager.getRepository(DHCPGroup), 'findOneOrFail')
        .throws();
    ...
    it('should correctly set the rule_order', async () => {
      ...
      const existingDHCPRule: DHCPRule = manager.getRepository(DHCPRule).create(
        manager.getRepository(DHCPRule).create({
          group: group,
          rule_order: 1,
          interface: null,
    ...
    it('should move the stored DHCPRule to a new position', async () => {
      ...
      expect(getLastDHCPRuleInGroupStub.calledOnce).to.be.true;
      expect(saveStub.calledOnce).to.be.true;
      expect(moveStub.calledOnceWith([expectedDHCPRule.id], data.to, data.offset as Offset)).to.be
        .true; // Cast 'data.offset' to 'Offset'
      expect(result).to.deep.equal(expectedDHCPRule);
    ...
    it('should throw an error for invalid IP version combination', async () => {
      ...
      const router: IPObj = await manager.getRepository(IPObj).save(
        manager.getRepository(IPObj).create({
          name: 'test',
          address: '0.0.0.0',
          ipObjTypeId: 1,
      ...
      const range: IPObj = await manager.getRepository(IPObj).save(
        manager.getRepository(IPObj).create({
          name: 'test',
          address: '0.0.0.0',
          ipObjTypeId: 2,
  ...
  describe('copy', () => {
    ...
    beforeEach(async () => {
      dhcpRule.group = await manager.getRepository(DHCPGroup).save(
        manager.getRepository(DHCPGroup).create({
          name: 'group',
          firewall: firewall,
        }),
  ...
  describe('update', () => {
    it('should successfully update a DHCPRule', async () => {
      const dhcpRule = await manager.getRepository(DHCPRule).save(
        manager.getRepository(DHCPRule).create({
          id: 1,
          group: await manager.getRepository(DHCPGroup).save(
            manager.getRepository(DHCPGroup).create({
    ...
    it('should handle exceptions in validateUpdateIpObjIds correctly', async () => {
      const dhcpRule: DHCPRule = await manager.getRepository(DHCPRule).save(
        manager.getRepository(DHCPRule).create({
          id: 1,
          rule_order: 1,
          interface: null,
      ...
      await expect(service.update(dhcpRule.id, { ipObjIds })).to.be.rejectedWith(
        Error,
        'Validation error',
      );
